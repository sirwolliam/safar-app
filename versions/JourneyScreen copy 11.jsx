...
const pl = StyleSheet.create({
  ...
  translitText:    { fontSize:typography.small, color:colors.subtext, fontStyle:"italic", lineHeight:typography.small*1.7, fontWeight:"400", paddingVertical:spacing(1) },
  transText:       { fontFamily:SERIF, fontSize:typography.body, color:colors.text, lineHeight:typography.body*1.65, fontWeight:"400", paddingVertical:spacing(1) },
  ...
  ctrlLabel:       { fontSize:typography.tiny, color:colors.subtext, fontWeight:"400" },
...
const sr = StyleSheet.create({
  ...
  sub:           { fontSize:typography.small, color:colors.subtext, fontWeight:"400" },
...
export default function JourneyScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={jn.safe}>
      ...
      {/* Hero — reduced to 140px, purely atmospheric */}
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }} contentContainerStyle={jn.scroll}>
        ...
        {/* ── 1. Umrah / Hajj selector ── */}
        <View style={jn.modeWrap}>
          <Text style={jn.modeSectionLabel}>I am preparing for</Text>
          <View style={jn.modeRow}>
            {[
              { key:"umrah", emoji:"\uD83D\uDD4B", name:"Umrah", sub:"Any time of year" },
              { key:"hajj",  emoji:"\u26F0\uFE0F",  name:"Hajj",  sub:"Dhul Hijjah" },
            ].map(m => (
        ...
        {/* ── 2. HERO CARD: Step-by-step Guide ──────────────────────────────
             Dominant card — full width, tall, image background, progress inside */}
        <TouchableOpacity style={jn.heroCard}
          ...
          <ImageBackground
            ...
            <View style={jn.heroCardContent}>
              <View style={jn.heroCardTop}>
                <View style={jn.heroCardBadge}>
                  <Text style={jn.heroCardBadgeTxt}>{mode === "umrah" ? "Umrah" : "Hajj"}</Text>
                </View>
                <Text style={jn.heroCardArrow}>{"\u203a"}</Text>
              </View>
              <Text style={jn.heroCardTitle}>Step-by-step Guide</Text>
              <Text style={jn.heroCardSub}>{"Track each step of your " + (mode === "umrah" ? "Umrah" : "Hajj")}</Text>
              {/* Progress bar */}
        ...
        {/* ── 3. FEATURED CARD: My Journey Board ────────────────────────────
             Personal + unique — medium tall card with preview stats */}
        <TouchableOpacity style={jn.boardCard}
          onPress={() => navigation?.navigate?.("MyBoard")}
          activeOpacity={0.88}>
          <View style={jn.boardCardLeft}>
            <View style={jn.boardCardIconWrap}>
              <Text style={jn.boardCardIcon}>{"\uD83D\uDCCC"}</Text>
            </View>
            <Text style={jn.boardCardTitle}>My Journey Board</Text>
            <Text style={jn.boardCardSub}>Your personal space for notes, duas, links and reminders</Text>
            {/* Stats row */}
            <View style={jn.boardStatsRow}>
              {boardCards.length === 0 ? (
                <Text style={jn.boardStatEmpty}>No cards yet — tap to add your first</Text>
              ) : (
                <>
                  <View style={jn.boardStat}>
                    <Text style={jn.boardStatNum}>{boardCards.length}</Text>
                    <Text style={jn.boardStatLabel}>cards</Text>
                  </View>
                  {boardChecklist > 0 ? (
                    <View style={jn.boardStat}>
                      <Text style={jn.boardStatNum}>{boardDone}/{boardChecklist}</Text>
                      <Text style={jn.boardStatLabel}>tasks</Text>
                    </View>
                  ) : null}
                </>
              )}
          ...
          <View style={jn.boardCardRight}>
            {/* Visual preview — stacked card thumbnails */}
            <View style={jn.boardPreview}>
              <View style={[jn.boardPreviewCard, { top:0, right:0,  backgroundColor:"#FFF9E8", borderColor:"#E8D9A0" }]} />
              <View style={[jn.boardPreviewCard, { top:8, right:8,  backgroundColor:"#EBF5EE", borderColor:"#C8DDD0" }]} />
              <View style={[jn.boardPreviewCard, { top:16, right:16, backgroundColor:"#FDFAF4", borderColor:"#E0D8C8" }]}>
                <Text style={jn.boardPreviewIcon}>{"\uD83D\uDCDD"}</Text>
              </View>
            </View>
          </View>
        </TouchableOpacity>

        {/* ── 4. HALF-WIDTH PAIR: Sacred Places + What to Expect ────────────
             Supporting content — equal peers, side by side */}
        <View style={jn.halfRow}>
          ...
          <TouchableOpacity style={jn.halfCard}
            ...
            activeOpacity={0.88}>
            <View style={jn.halfCardImgWrap}>
              {/* Placeholder: replace with kaaba_map.png crop */}
              <View style={jn.halfCardImgPlaceholder}>
                <Text style={jn.halfCardPlaceholderEmoji}>{"\uD83D\uDDFA\uFE0F"}</Text>
              </View>
            </View>
            <View style={jn.halfCardBody}>
              <Text style={jn.halfCardTitle}>Sacred Places</Text>
              <Text style={jn.halfCardSub}>Duas for each location</Text>
              <Text style={jn.halfCardArrow}>{"\u203a"}</Text>
            </View>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={jn.halfCard}
            ...
            <View style={jn.halfCardImgWrap}>
              <View style={jn.halfCardImgPlaceholder}>
                <Text style={jn.halfCardPlaceholderEmoji}>{"\uD83D\uDCD6"}</Text>
              </View>
            </View>
            <View style={jn.halfCardBody}>
              <Text style={jn.halfCardTitle}>What to Expect</Text>
              <Text style={jn.halfCardSub}>Plan & prepare</Text>
              <Text style={jn.halfCardArrow}>{"\u203a"}</Text>
            </View>
          </TouchableOpacity>
        ...
        {/* ── 5. COMPACT ROW: Groups + Contacts ─────────────────────────────
             Utility / social — 2-column compact cards */}
        <View style={jn.compactRow}>
          ...
          <TouchableOpacity style={jn.compactCard}
            ...
            <View style={{ flex:1 }}>
              ...
              <Text style={jn.compactSub}>Share milestones</Text>
            </View>
            <Text style={jn.compactArrow}>{"\u203a"}</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={jn.compactCard}
            ...
            <View style={{ flex:1 }}>
              ...
              <Text style={jn.compactSub}>{contacts.length > 0 ? contacts.length + " saved" : "Hotel, guide & more"}</Text>
            </View>
            <Text style={jn.compactArrow}>{"\u203a"}</Text>
          </TouchableOpacity>
...
const jn = StyleSheet.create({
  ...
  heroScrim: { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(0,0,0,0.18)" },
  ...
  modeSectionLabel:{ fontSize:11, fontWeight:"700", letterSpacing:1.5, color:colors.subtext, textTransform:"uppercase", marginBottom:spacing(1) },
  modeRow:         { flexDirection:"row", gap:spacing(1.25) },
  modeOpt:         { flex:1, alignItems:"center", paddingVertical:spacing(2), borderRadius:radius.md, gap:5, backgroundColor:colors.card, borderWidth:1.5, borderColor:colors.border, ...shadows.card },
  modeOptActive:   { borderColor:colors.primary, backgroundColor:"rgba(47,93,80,0.06)" },
  modeEmoji:       { fontSize:26 },
  modeName:        { fontFamily:SERIF, fontSize:typography.body, color:colors.subtext },
  modeNameActive:  { color:colors.primary, fontWeight:"500" },
  modeSub:         { fontSize:11, color:colors.subtext, fontWeight:"400", textAlign:"center" },
  modeSubActive:   { color:colors.primary },

  // ── 2. Hero card: Step-by-step Guide ─────────────────────────────────────────
  heroCard:        { borderRadius:radius.xl, overflow:"hidden", marginBottom:spacing(1.5), height:200, ...shadows.card },
  heroCardBg:      { flex:1, justifyContent:"flex-end" },
  heroCardScrim:   { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.55)" },
  heroCardContent: { padding:spacing(2.5) },
  heroCardTop:     { flexDirection:"row", justifyContent:"space-between", alignItems:"center", marginBottom:spacing(1) },
  heroCardBadge:   { backgroundColor:"rgba(255,255,255,0.18)", borderRadius:radius.pill, paddingHorizontal:spacing(1.5), paddingVertical:4, borderWidth:1, borderColor:"rgba(255,255,255,0.3)" },
  heroCardBadgeTxt:{ fontSize:12, color:"#fff", fontWeight:"600", letterSpacing:0.5 },
  heroCardArrow:   { fontSize:24, color:"rgba(255,255,255,0.6)" },
  heroCardTitle:   { fontFamily:SERIF, fontSize:24, color:"#fff", fontWeight:"400", marginBottom:4 },
  ...
  heroProgSeg:     { flex:1, height:"100%", borderRadius:3, backgroundColor:"rgba(255,255,255,0.2)" },
  heroProgSegFill: { backgroundColor:"#fff" },
  heroProgLabel:   { fontSize:13, color:"rgba(255,255,255,0.8)", fontWeight:"500" },

  // ── 3. Board card ─────────────────────────────────────────────────────────────
  boardCard:       { flexDirection:"row", backgroundColor:colors.card, borderRadius:radius.xl, borderWidth:1, borderColor:colors.border, padding:spacing(2.5), marginBottom:spacing(1.5), minHeight:140, ...shadows.card },
  boardCardLeft:   { flex:1, justifyContent:"space-between" },
  boardCardIconWrap:{ width:44, height:44, borderRadius:radius.md, backgroundColor:"rgba(47,93,80,0.1)", alignItems:"center", justifyContent:"center", marginBottom:spacing(1) },
  boardCardIcon:   { fontSize:22 },
  boardCardTitle:  { fontFamily:SERIF, fontSize:20, color:colors.text, marginBottom:4 },
  boardCardSub:    { fontSize:13, color:colors.subtext, lineHeight:18, marginBottom:spacing(1.5) },
  boardStatsRow:   { flexDirection:"row", gap:spacing(2) },
  boardStat:       { alignItems:"flex-start" },
  boardStatNum:    { fontFamily:SERIF, fontSize:20, color:colors.primary, fontWeight:"500" },
  boardStatLabel:  { fontSize:11, color:colors.subtext },
  boardStatEmpty:  { fontSize:12, color:colors.subtext, fontStyle:"italic" },
  boardCardRight:  { width:100, alignItems:"flex-end", justifyContent:"flex-start" },
  boardPreview:    { width:80, height:90, position:"relative" },
  boardPreviewCard:{ position:"absolute", width:64, height:80, borderRadius:radius.md, borderWidth:1, ...shadows.card },
  boardPreviewIcon:{ fontSize:24, position:"absolute", bottom:8, left:12 },

  // ── 4. Half-width cards: Sacred Places + What to Expect ──────────────────────
  halfRow:              { flexDirection:"row", gap:spacing(1.25), marginBottom:spacing(1.5) },
  halfCard:             { flex:1, backgroundColor:colors.card, borderRadius:radius.lg, borderWidth:1, borderColor:colors.border, overflow:"hidden", ...shadows.card },
  halfCardImgWrap:      { width:"100%", height:110 },
  halfCardImgPlaceholder:{ width:"100%", height:"100%", backgroundColor:"#DDD8C8", alignItems:"center", justifyContent:"center" },
  halfCardPlaceholderEmoji:{ fontSize:36 },
  halfCardBody:         { padding:spacing(1.5) },
  halfCardTitle:        { fontFamily:SERIF, fontSize:15, color:colors.text, marginBottom:3 },
  halfCardSub:          { fontSize:12, color:colors.subtext, marginBottom:spacing(0.75) },
  halfCardArrow:        { fontSize:16, color:colors.primary },

  // ── 5. Compact 2-column row: Groups + Contacts ───────────────────────────────
  compactRow:      { flexDirection:"row", gap:spacing(1.25), marginBottom:spacing(1.5) },
  compactCard:     { flex:1, flexDirection:"row", alignItems:"center", gap:spacing(1.25), backgroundColor:colors.card, borderRadius:radius.lg, borderWidth:1, borderColor:colors.border, padding:spacing(1.5), ...shadows.card },
  compactIconWrap: { width:40, height:40, borderRadius:radius.md, backgroundColor:"rgba(47,93,80,0.08)", alignItems:"center", justifyContent:"center", flexShrink:0 },
  compactTitle:    { fontFamily:SERIF, fontSize:14, color:colors.text, marginBottom:2 },
  compactSub:      { fontSize:11, color:colors.subtext },
  compactArrow:    { fontSize:16, color:colors.border },
