  ...
  return (
    <SafeAreaView style={jn.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }} contentContainerStyle={jn.scroll}>
        ...
        <TouchableOpacity style={jn.heroCard}
          ...
          <ImageBackground
            ...
            <View style={jn.heroCardContent}>
              {/* Badge — top left, inverted (white bg, green text), bigger */}
              <View style={jn.heroCardBadgeWrap}>
                <View style={jn.heroCardBadge}>
                  <Text style={jn.heroCardBadgeTxt}>{mode === "umrah" ? "Umrah" : "Hajj"}</Text>
                </View>
              </View>
              {/* Title + sub pushed to bottom */}
        ...
        {/* ── 3. My Journey Board — image background with text overlay ── */}
        <TouchableOpacity style={jn.boardCard}
          onPress={() => navigation?.navigate?.("MyBoard")}
          activeOpacity={0.88}>
          {/* Placeholder background — warm parchment tones */}
          <View style={jn.boardCardBg}>
            {/* Decorative stacked card shapes — suggest board content */}
            <View style={jn.boardDecorCard1} />
            <View style={jn.boardDecorCard2} />
            <View style={jn.boardDecorCard3} />
          </View>
          {/* Dark bottom banner overlay */}
          <View style={jn.boardBanner}>
            <View style={jn.boardBannerLeft}>
              <Text style={jn.boardBannerTitle}>My Journey Board</Text>
              <Text style={jn.boardBannerSub}>Notes, duas, links & reminders</Text>
            </View>
            {/* Stats */}
            {boardCards.length > 0 ? (
              <View style={jn.boardBannerStats}>
                <View style={jn.boardBannerStat}>
                  <Text style={jn.boardBannerStatNum}>{boardCards.length}</Text>
                  <Text style={jn.boardBannerStatLbl}>cards</Text>
                </View>
                {boardChecklist > 0 ? (
                  <View style={jn.boardBannerStat}>
                    <Text style={jn.boardBannerStatNum}>{boardDone}/{boardChecklist}</Text>
                    <Text style={jn.boardBannerStatLbl}>tasks</Text>
                  </View>
                ) : null}
              </View>
            ) : (
              <Text style={jn.boardBannerEmpty}>Tap to start{"\u203a"}</Text>
            )}
          </View>
        </TouchableOpacity>
...
const jn = StyleSheet.create({
  ...
  // ── 1. Mode selector ─────────────────────────────────────────────────────────
  modeWrap:        { marginBottom:spacing(2) },
  modeSectionLabel:{ fontSize:11, fontWeight:"700", letterSpacing:1.5, color:colors.subtext, textTransform:"uppercase", textAlign:"center", marginBottom:spacing(1.5) },
  ...
  // ── 2. Hero card: Step-by-step Guide ─────────────────────────────────────────
  heroCard:         { borderRadius:radius.xl, overflow:"hidden", marginBottom:spacing(1.5), height:210, ...shadows.card },
  ...
  heroCardContent:  { flex:1, justifyContent:"space-between" },
  // Badge — top left, inverted: white bg, green text, bigger
  heroCardBadgeWrap:{ padding:spacing(2) },
  heroCardBadge:    { alignSelf:"flex-start", backgroundColor:"#fff", borderRadius:radius.pill, paddingHorizontal:spacing(2), paddingVertical:spacing(0.875) },
  heroCardBadgeTxt: { fontFamily:SERIF, fontSize:15, color:colors.primary, fontWeight:"600" },
  // Bottom section
  ...
  heroCardSub:      { fontSize:14, color:"rgba(255,255,255,0.75)", fontWeight:"400", marginBottom:spacing(1.75) },
  ...
  // ── 3. Board card — image bg + dark bottom banner ─────────────────────────────
  boardCard:           { borderRadius:radius.xl, overflow:"hidden", marginBottom:spacing(1.5), height:180, ...shadows.card },
  boardCardBg:         { ...StyleSheet.absoluteFillObject, backgroundColor:"#C8B89A" },
  // Decorative layered card shapes — suggest board content
  boardDecorCard1:     { position:"absolute", top:12, left:16,  width:90, height:110, borderRadius:radius.md, backgroundColor:"rgba(255,255,255,0.22)", transform:[{ rotate:"-6deg" }] },
  boardDecorCard2:     { position:"absolute", top:8,  left:52,  width:90, height:110, borderRadius:radius.md, backgroundColor:"rgba(255,255,255,0.16)", transform:[{ rotate:"2deg"  }] },
  boardDecorCard3:     { position:"absolute", top:4,  right:20, width:90, height:110, borderRadius:radius.md, backgroundColor:"rgba(255,255,255,0.30)", transform:[{ rotate:"8deg"  }] },
  // Dark bottom banner overlay
  boardBanner:         { position:"absolute", bottom:0, left:0, right:0, backgroundColor:"rgba(15,12,8,0.72)", paddingHorizontal:spacing(2.5), paddingVertical:spacing(1.75), flexDirection:"row", alignItems:"center", justifyContent:"space-between" },
  boardBannerLeft:     { flex:1 },
  boardBannerTitle:    { fontFamily:SERIF, fontSize:19, color:"#fff", fontWeight:"400", marginBottom:2 },
  boardBannerSub:      { fontSize:12, color:"rgba(255,255,255,0.65)" },
  boardBannerStats:    { flexDirection:"row", gap:spacing(2.5), alignItems:"center" },
  boardBannerStat:     { alignItems:"center" },
  boardBannerStatNum:  { fontFamily:SERIF, fontSize:28, color:"#fff", fontWeight:"400", lineHeight:32 },
  boardBannerStatLbl:  { fontSize:10, color:"rgba(255,255,255,0.6)", letterSpacing:0.5 },
  boardBannerEmpty:    { fontFamily:SERIF, fontSize:15, color:"rgba(255,255,255,0.7)" },

  // ── 4. Half cards — full image with text overlay ──────────────────────────────
  halfRow:           { flexDirection:"row", gap:spacing(1.25), marginBottom:spacing(1.5) },
  halfCard:          { flex:1, borderRadius:radius.lg, overflow:"hidden", height:150, ...shadows.card },
  halfCardImg:       { flex:1, justifyContent:"flex-end" },
  halfCardScrim:     { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.45)" },
  halfCardOverlay:   { padding:spacing(1.5) },
  halfCardTitleWhite:{ fontFamily:SERIF, fontSize:15, color:"#fff", fontWeight:"400", marginBottom:2 },
  halfCardSubWhite:  { fontSize:11, color:"rgba(255,255,255,0.75)" },
  ...
  // Legacy link card styles — kept for any remaining refs
