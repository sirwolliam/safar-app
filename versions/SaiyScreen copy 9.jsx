    ...
    <SafeAreaView style={[s.safe, { paddingBottom:insets.bottom }]}>
      ...
      <View style={[s.graphicWrap, { width:GRAPHIC_W }]}>
        ...
        <View style={s.middleRow}>
          ...
          {/* Left — downward flow (Marwah → Safa) */}
          <View style={s.arrowWrap}>
            {[1,0.65,0.38,0.18,0.07].map((op,i) => (
              <Text key={i} style={[s.arrowChev, {
                opacity: destination === "SAFA" ? op : op * 0.25,
                color: GREEN_LITE,
              }]}>{"‹"}</Text>
            ))}
          </View>
          ...
          {/* Right — upward flow (Safa → Marwah) */}
          <View style={s.arrowWrap}>
            {[0.07,0.18,0.38,0.65,1].map((op,i) => (
              <Text key={i} style={[s.arrowChev, {
                opacity: destination === "MARWAH" ? op : op * 0.25,
                color: GREEN_LITE,
              }]}>{"›"}</Text>
            ))}
          </View>
