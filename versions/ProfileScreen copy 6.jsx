...
export default function ProfileScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {/* Category chips — two rows, tap to jump to section */}
      <View style={s.chipsWrap}>
        {CATEGORIES.map(cat => (
          <TouchableOpacity key={cat.key} style={s.chip}
            onPress={() => scrollTo(cat.key)} activeOpacity={0.8}>
            <Text style={s.chipTxt}>{cat.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
...
const createStyles = (colors) => StyleSheet.create({
  ...
  // Category chips — two natural rows, left-aligned
  chipsWrap: { flexDirection:"row", flexWrap:"wrap", paddingHorizontal:spacing(2.5), paddingBottom:spacing(1.5), gap:spacing(0.875) },
  chip:      { paddingHorizontal:spacing(1.75), paddingVertical:spacing(1), borderRadius:radius.pill, backgroundColor:colors.card, borderWidth:1, borderColor:colors.border, ...shadows.card },
  chipTxt:   { fontFamily:SERIF, fontSize:13, color:colors.primary, fontWeight:"400" },
