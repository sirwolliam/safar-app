 ...
 * Hub screen for all Umrah content.
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        ...
        <View style={s.headerCenter}>
          <Text style={s.headerTitle}>My Umrah</Text>
          <Text style={s.headerSub}>Your complete Umrah companion</Text>
        </View>
        <View style={{ width:36 }} />
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        ...
        >
          ...
          <View style={s.heroContent}>
            <Text style={s.heroEyebrow}>UMRAH GUIDE</Text>
        ...
        <View style={s.sectionDivider}>
          ...
          <Text style={s.sectionLabel}>MY UMRAH TOOLS</Text>
        ...
        <TouchableOpacity style={s.hubCard} onPress={() => setShowCarousel(true)} activeOpacity={0.88}>
          ...
          <View style={s.hubText}>
            ...
            <Text style={s.hubSub}>Visual walkthrough of all 7 Umrah rites</Text>
          </View>
          <Text style={s.hubArrow}>{"›"}</Text>
        </TouchableOpacity>

        {/* Journey Map */}
        <TouchableOpacity style={s.hubCard} onPress={() => navigation?.navigate?.("PilgrimageMap", { mode:"umrah" })} activeOpacity={0.88}>
          <View style={[s.hubIconBox, { backgroundColor:"#2A5242" }]}>
            <Text style={s.hubIcon}>{"\uD83D\uDDFA\uFE0F"}</Text>
          </View>
          <View style={s.hubText}>
            <Text style={s.hubTitle}>Journey Map</Text>
            <Text style={s.hubSub}>Interactive route map with du\u02bf\u0101\u02bes for every stop</Text>
          </View>
          <Text style={s.hubArrow}>{"›"}</Text>
        </TouchableOpacity>

        {/* Du'ās */}
        <TouchableOpacity style={s.hubCard} onPress={() => navigation?.navigate?.("Duas")} activeOpacity={0.88}>
          <View style={[s.hubIconBox, { backgroundColor:"#3B6B58" }]}>
            <Text style={s.hubIcon}>{"\uD83E\uDD0D"}</Text>
          </View>
          <View style={s.hubText}>
            <Text style={s.hubTitle}>{"Du\u02bf\u0101\u02bes"}</Text>
            <Text style={s.hubSub}>Authenticated du\u02bf\u0101\u02bes for every moment of Umrah</Text>
          </View>
          <Text style={s.hubArrow}>{"›"}</Text>
        </TouchableOpacity>

        {/* ── Checklist (inline, expandable) ── */}
        <View style={s.sectionDivider}>
          <View style={s.sectionBar} />
          <Text style={s.sectionLabel}>MY UMRAH CHECKLIST</Text>
          <View style={s.sectionLine} />
          <Text style={s.sectionCount}>{completedCount}/{UMRAH_CHECKLIST.length}</Text>
        </View>
...
const s = StyleSheet.create({
  ...
  hero:        { height:200, marginBottom:spacing(2), borderRadius:16, overflow:"hidden", ...shadows.card },
  ...
  heroTitle:   { fontFamily:SERIF, fontSize:26, color:"#FDFAF4", fontWeight:"400", lineHeight:33, marginBottom:6 },
  heroSub:     { fontSize:13, color:"rgba(253,250,244,0.75)", lineHeight:19 },
  ...
  hubIcon:   { fontSize:20 },
  hubText:   { flex:1 },
  hubTitle:  { fontFamily:SERIF, fontSize:16, color:colors.text, fontWeight:"600", marginBottom:2 },
  hubSub:    { fontSize:12, color:colors.subtext, lineHeight:17 },
