...
function Slide({ step, total }) {
  return (
    <View style={sl.card}>
      {/* Full card image — 75% of card height */}
      <Image source={step.image} style={sl.img} resizeMode="cover" />

      {/* Top-to-bottom gradient scrim — light at top, darker toward text */}
      <View style={sl.gradientTop} />
      <View style={sl.gradientBottom} />

      {/* Step number — top centre over image */}
      <View style={sl.badge}>
        <Text style={sl.badgeNum}>{step.num}</Text>
        <Text style={sl.badgeTotal}>{"/" + total}</Text>
      </View>

      {/* Text panel — bottom 25%, sits on card background */}
      <View style={sl.content}>
        ...
        <Text style={sl.body} numberOfLines={3}>{step.body}</Text>
        <Text style={sl.sourceNote}>{"Ṣaḥīḥ al-Bukhārī · Ṣaḥīḥ Muslim · Sunan Abī Dāwūd · Consult a scholar for your madhab"}</Text>
      </View>
    </View>
  );
}
...
const sl = StyleSheet.create({
  card: {
    ...
    backgroundColor:"#FDFAF4", borderRadius:24, overflow:"hidden",
    shadowColor:"#1A1712", shadowOffset:{width:0,height:8}, shadowOpacity:0.2, shadowRadius:24, elevation:12,
  },
  // Full bleed image — takes 75% of card
  img: {
    position:"absolute", top:0, left:0, right:0,
    height:CARD_H * 0.75,
  },
  // Gradient layers — stacked to simulate top-to-bottom fade
  // Very subtle at top, building to soft at bottom where image meets text panel
  gradientTop: {
    position:"absolute", top:0, left:0, right:0, height:CARD_H * 0.18,
    backgroundColor:"rgba(10,8,4,0.08)",
  },
  gradientBottom: {
    position:"absolute",
    top:CARD_H * 0.48,
  ...
  badge: {
    ...
    flexDirection:"row", justifyContent:"center", alignItems:"baseline", gap:3,
  },
  badgeNum:   { fontFamily:SERIF, fontSize:40, color:"#fff", lineHeight:46, textShadowColor:"rgba(0,0,0,0.22)", textShadowOffset:{width:0,height:1}, textShadowRadius:8 },
  badgeTotal: { fontFamily:SERIF, fontSize:15, color:"rgba(255,255,255,0.6)", textShadowColor:"rgba(0,0,0,0.2)", textShadowOffset:{width:0,height:1}, textShadowRadius:4 },
  // Text content — bottom 25% on solid card background
  content: {
    position:"absolute",
  ...
  subtitle: { fontSize:10, fontWeight:"700", letterSpacing:2, color:"#2F5D50", textAlign:"center", marginBottom:6 },
  divider:  { width:24, height:1.5, backgroundColor:"#C8A96A", marginBottom:8 },
  title:    { fontFamily:SERIF, fontSize:20, color:"#1A1712", textAlign:"center", marginBottom:6, lineHeight:26 },
  body:     { fontSize:13, color:"#5A5650", lineHeight:19, textAlign:"center" },
});
...
const gc = StyleSheet.create({
  ...
  progressLabel: { fontSize:12, color:"rgba(255,255,255,0.80)", fontWeight:"600", letterSpacing:0.5 },
  progressTrack: { width:"100%", height:3, backgroundColor:"rgba(255,255,255,0.2)", borderRadius:2, overflow:"hidden" },
