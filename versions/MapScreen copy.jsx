    ...
    <View style={mi.wrap}>
      ...
      {sites.map(site => {
        ...
        return (
          <TouchableOpacity
            key={site.id}
            style={{ left, top }, sel ? [mi.pin, mi.pinActive] : mi.pin}
            onPress={() => onSelect(site)}
            activeOpacity={0.8}
          >
            <View style={sel ? [mi.pinInner, mi.pinInnerActive] : mi.pinInner} />
          </TouchableOpacity>
        );
      })}
    </View>
...
const sc = StyleSheet.create({
  ...
  subtitle:   { fontSize:typography.small, color:colors.subtext, fontWeight:"400", marginBottom:spacing(1) },
  description:{ fontSize:typography.small, color:colors.subtext, fontWeight:"400", lineHeight:typography.small*1.65, marginBottom:spacing(1.5), paddingTop:spacing(1), borderTopWidth:1, borderTopColor:colors.border },
  ...
  countLabel: { fontSize:typography.small, color:colors.subtext, fontWeight:"400" },
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={chip.row}>
      {sites.map(s => {
        ...
        return (
          <TouchableOpacity key={s.id} style={on ? [chip.pill, chip.pillOn] : chip.pill} onPress={() => onSelect(s)} activeOpacity={0.8}>
            <Text style={on ? [chip.label, chip.labelOn] : chip.label}>{s.name}</Text>
          </TouchableOpacity>
        );
      })}
    </ScrollView>
...
const chip = StyleSheet.create({
  ...
  label:  { fontSize:typography.small, color:colors.subtext, fontWeight:"400" },
  ...
  return (
    <View style={lst.section}>
      ...
      {sites.map((s, i) => {
        const on = s.id === selectedId;
        return (
          <TouchableOpacity key={s.id} style={i===sites.length-1 ? [lst.row, lst.rowLast] : lst.row}
            onPress={() => onSelect(s)} activeOpacity={0.85}>
            <View style={on ? [lst.dot, lst.dotOn] : lst.dot} />
            <View style={lst.info}>
              <Text style={on ? [lst.name, lst.nameOn] : lst.name}>{s.name}</Text>
              <Text style={lst.sub}>{s.subtitle}</Text>
            </View>
            <Text style={on ? [lst.count, lst.countOn] : lst.count}>{s.duaCount} duas</Text>
...
const lst = StyleSheet.create({
  ...
  count:        { fontSize:typography.small, color:colors.subtext, fontWeight:"400" },
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.cityToggle}>
        {["Makkah","Madinah"].map(c => {
          const on = c === city;
          return (
            <TouchableOpacity key={c} style={on ? [styles.cityOpt, styles.cityOptOn] : styles.cityOpt}
              onPress={() => handleCitySwitch(c)} activeOpacity={0.8}>
              <Text style={on ? [styles.cityLabel, styles.cityLabelOn] : styles.cityLabel}>{c}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  cityLabel:  { fontSize:typography.body, color:colors.subtext, fontWeight:"400" },
  ...
  madinahNoticeText: { fontSize:typography.small, color:colors.primary, fontWeight:"400", lineHeight:typography.small*1.6 },
});
