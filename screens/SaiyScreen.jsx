...
import { ArrowLeft, Check, X, MapPin, Play, Stop } from "phosphor-react-native";
...
function DuaAudioPlayer({ duaName }) {
  ...
  const handlePlay = () => {
    if (playing) {
      ...
      showComingSoon();
      // Simulate playback progress for UI preview
      setPlaying(true);
...
const dl = StyleSheet.create({
  ...
  translit:   { fontSize:16, color:GOLD, textAlign:"center", fontStyle:"italic", marginBottom:14 },
  ...
  return (
    <SafeAreaView style={[s.safe, { paddingBottom:insets.bottom }]}>
      ...
      <View style={[s.graphicWrap, { width:GRAPHIC_W }]}>
        ...
        {/* Dashed oval track — behind everything */}
        <Svg
        ...
        {/* SAFA label + mountain — sits directly above button */}
        <View style={s.locationTop}>
          <Text style={s.locationLabel}>SAFA</Text>
          <Mountain size={64} filled={completed >= 2} color={GREEN_LITE} />
        </View>

        {/* Side arrow flows + centre button */}
        <View style={s.middleRow}>

          {/* Left — downward chevrons (↓), active when heading to Safa */}
          <View style={s.arrowWrap}>
            {[0.15, 0.35, 0.60, 0.85, 1].map((op, i) => (
              <Text key={i} style={[s.arrowChev, {
                opacity: destination === "SAFA" ? 0.90 : 0.18,
                color: GREEN_LITE,
              }]}>{"∨"}</Text>
            ))}
          </View>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <View style={s.centreBtnTop}>
              <MapPin size={28} color={WHITE} weight="fill" />
              <Text style={s.centreBtnLabel}>{done ? "Alhamdulillah!" : "I have reached"}</Text>
              <Text style={[s.centreBtnDest, done && { fontSize:18, letterSpacing:0, fontWeight:"500" }]}>
          ...
          {/* Right — upward chevrons (↑), active when heading to Marwah */}
          <View style={s.arrowWrap}>
            {[1, 0.85, 0.60, 0.35, 0.15].map((op, i) => (
              <Text key={i} style={[s.arrowChev, {
                opacity: destination === "MARWAH" ? 0.90 : 0.18,
                color: GREEN_LITE,
              }]}>{"∧"}</Text>
            ))}
          </View>
        ...
        {/* MARWAH mountain + label — sits directly below button */}
        <View style={s.locationBottom}>
          <Mountain size={64} filled={completed >= 1} color={GREEN_LITE} />
...
const s = StyleSheet.create({
  ...
  dotsLabel:    { fontSize:13, color:GOLD, fontWeight:"500", marginTop:8 },

  // Dua cards
  duasRow:       { flexDirection:"row", marginHorizontal:spacing(2.5), gap:spacing(1.25), marginBottom:spacing(2), marginTop:spacing(1) },
  duaCard:       { flex:1, backgroundColor:CARD, borderRadius:14, borderWidth:1, borderColor:"rgba(90,140,74,0.30)", padding:16 },
