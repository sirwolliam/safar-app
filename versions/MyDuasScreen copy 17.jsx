...
const TAB_ITEMS = [
  { key:"myLists", label:"My Dua Lists",  sub:"Your saved lists", img:require("../assets/tab_dua_library.jpg")  },
  { key:"library", label:"Dua Library",   sub:"Browse all duas",  img:require("../assets/tab_my_lists.jpg")     },
  { key:"shared",  label:"Shared Duas",   sub:"Shared with me",   img:require("../assets/tab_shared_duas.jpg")  },
];
        ...
        return (
          <TouchableOpacity key={t.key}
            ...
            <View style={tb.textWrap}>
              <Text style={isOn ? [tb.label, tb.labelOn] : tb.label} numberOfLines={1}>{t.label}</Text>
              <Text style={isOn ? [tb.sub, tb.subOn] : tb.sub} numberOfLines={1}>{t.sub}</Text>
            </View>
...
const tb = StyleSheet.create({
  ...
  btnOn:    { borderColor:colors.primary, borderWidth:2 },

  // Image area — top of button
  imgWrap:  { width:"100%", height:72, position:"relative" },
  img:      { width:"100%", height:"100%" },
  imgScrim: { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(47,93,80,0.25)" },

  // Text area — below image
  textWrap: { padding:spacing(1), paddingTop:spacing(0.875) },
  label:    { fontFamily:SERIF, fontSize:12, color:colors.text, lineHeight:16, marginBottom:1 },
  labelOn:  { color:colors.primary, fontWeight:"600" },
  sub:      { fontSize:10, color:colors.subtext, lineHeight:14 },
      ...
      <KeyboardAvoidingView style={{ flex:1 }} behavior={Platform.OS==="ios"?"padding":"height"}>
        <View style={nl.overlay}>
          <View style={nl.sheet}>
            ...
            <Text style={nl.fieldLabel}>CHOOSE AN ICON</Text>
            <View style={nl.emojiRow}>
              {EMOJIS.map(e => (
                <TouchableOpacity key={e}
                  style={emoji === e ? [nl.emojiBtn, nl.emojiBtnOn] : nl.emojiBtn}
                  onPress={() => setEmoji(e)} activeOpacity={0.8}>
                  <Text style={nl.emojiTxt}>{e}</Text>
                </TouchableOpacity>
              ))}
            </View>

            {/* Preview of the new list card */}
            {name.trim() ? (
              <View style={nl.preview}>
                <View style={[nl.previewThumb, { backgroundColor:colors.primary }]}>
            ...
            <View style={nl.btnRow}>
              <TouchableOpacity style={nl.cancelBtn} onPress={() => { setName(""); setEmoji("\uD83D\uDD4B"); onClose(); }}>
                <Text style={nl.cancelTxt}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={!name.trim() ? [nl.createBtn, { opacity:0.4 }] : nl.createBtn}
                onPress={() => { setName(""); setEmoji("\uD83D\uDD4B"); onClose(); }} disabled={!name.trim()}>
...
const nl = StyleSheet.create({
  ...
  emojiRow:    { flexDirection:"row", gap:spacing(1), marginBottom:spacing(2), flexWrap:"wrap" },
  ...
  emojiBtnOn:  { borderColor:colors.primary, borderWidth:2, backgroundColor:"rgba(47,93,80,0.08)" },
  emojiTxt:    { fontSize:22 },
  preview:     { flexDirection:"row", alignItems:"center", gap:spacing(1.5), backgroundColor:colors.card, borderRadius:radius.md, borderWidth:1, borderColor:colors.border, padding:spacing(1.5), marginBottom:spacing(2), ...shadows.card },
  previewThumb:{ width:44, height:44, borderRadius:radius.sm, alignItems:"center", justifyContent:"center", flexShrink:0 },
  previewName: { fontFamily:SERIF, fontSize:15, fontWeight:"500", color:colors.text, marginBottom:2 },
