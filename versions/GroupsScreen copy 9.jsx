...
export default function GroupsScreen({ navigation }) {
  ...
  const groupAccent    = GROUP_COLORS.find(c => c.key === (activeGroup?.colorKey ?? "green"))?.value ?? "#2F5D50";
      ...
      <View style={s.tabsWrap}>
        <FlatList
          ...
          renderItem={({ item:g }) => {
            ...
            const accent  = GROUP_COLORS.find(c => c.key === (g.colorKey ?? "green"))?.value ?? "#2F5D50";
            return (
            ...
            <View style={mo.sheet} onStartShouldSetResponder={() => true}>
              ...
              {showLinkField && (
                <View style={mo.linkFields}>
                  <TextInput style={mo.linkInput} placeholder="https://..." placeholderTextColor={"#8A7E6A"}
                    value={postLink} onChangeText={setPostLink} keyboardType="url" autoCapitalize="none" autoCorrect={false} />
                  <TextInput style={mo.linkInput} placeholder="Link title (optional)" placeholderTextColor={"#8A7E6A"}
          ...
          <View style={ma.sheet} onStartShouldSetResponder={() => true}>
            ...
            {allGroups.filter(g => g.id !== activeId).map(g => (
              <TouchableOpacity key={g.id} style={ma.optionRow}
                ...
                <View style={ma.optionIcon}><GroupIcon size={16} color={"#2F5D50"} /></View>
...
const createStyles = (colors) => StyleSheet.create({
  ...
  newBtn:     { backgroundColor:"#2F5D50", borderRadius:radius.pill, paddingHorizontal:spacing(1.75), paddingVertical:spacing(0.875), shadowColor:"#2A2218", shadowOffset:{width:0,height:2}, shadowOpacity:0.18, shadowRadius:6, elevation:4 },
  ...
  shareBtn:     { borderRadius:radius.pill, paddingHorizontal:spacing(1.75), paddingVertical:spacing(0.875), shadowColor:"#2A2218", shadowOffset:{width:0,height:2}, shadowOpacity:0.18, shadowRadius:6, elevation:4 },
...
const mo = StyleSheet.create({
  ...
  submit:         { flex:1, borderRadius:radius.md, backgroundColor:"#2F5D50", paddingVertical:spacing(1.75), alignItems:"center", shadowColor:"#2A2218", shadowOffset:{width:0,height:2}, shadowOpacity:0.18, shadowRadius:6, elevation:4 },
