  ...
  return (
    <SafeAreaView style={jn.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }} contentContainerStyle={jn.scroll}>
        ...
        {/* ── 1. My Journey Board — hero card (height reduced 25%) ── */}
        <TouchableOpacity style={jn.boardHero}
          onPress={() => navigation?.navigate?.("MyBoard")}
          activeOpacity={0.88}>
          <ImageBackground
            source={require("../assets/myboard.jpg")}
            style={jn.boardHeroBg}
            imageStyle={{ resizeMode:"cover" }}>
            <View style={jn.boardHeroScrim} />
            <View style={jn.boardHeroContent}>
        ...
        {/* ── 3. Guide card — opens carousel directly ── */}
        <TouchableOpacity style={jn.guideCard}
          onPress={() => setShowCarousel(true)}
          activeOpacity={0.92}>
          <ImageBackground
            source={require("../assets/kaaba_mixed.png")}
            style={jn.guideCardBg}
            imageStyle={{ resizeMode:"cover" }}>
            <View style={jn.guideCardScrim} />
            <View style={jn.guideCardContent}>
              <View style={jn.guideCardTopRow}>
                <View style={jn.guideCardBadge}>
                  <Text style={jn.guideCardBadgeTxt}>{mode === "umrah" ? "UMRAH" : "HAJJ"}</Text>
                </View>
                <Text style={jn.guideCardArrow}>{"›"}</Text>
              </View>
              <View style={jn.guideCardBottom}>
                <Text style={jn.guideCardTitle}>{mode === "umrah" ? "Umrah Guide" : "Hajj Guide"}</Text>
                <Text style={jn.guideCardSub}>{"Complete step-by-step " + (mode === "umrah" ? "Umrah" : "Hajj") + " walkthrough"}</Text>
              </View>
            </View>
          </ImageBackground>
        </TouchableOpacity>

        {/* ── 4. Sacred Places + What to Expect — full width row ── */}
        <View style={jn.fullRow}>

          <TouchableOpacity style={jn.fullRowCard}
            onPress={() => navigation?.navigate?.("Map")}
            activeOpacity={0.88}>
            <ImageBackground
              source={require("../assets/kaaba_map.png")}
              style={jn.fullRowCardImg}
              imageStyle={{ resizeMode:"cover" }}>
        ...
        {/* ── 5. Checklist dropdown ── */}
        <TouchableOpacity style={jn.checklistHeader} onPress={() => setChecklistOpen(v => !v)} activeOpacity={0.85}>
          <ImageBackground
            source={mode === "hajj" ? require("../assets/04_tawaf_gradient.jpg") : require("../assets/Umrah_03_tawaf_gradient.jpg")}
        ...
        {checklistOpen && (
          <View style={jn.checklistBody}>
            {activeChecklist.map((item, idx) => (
              <View key={item.id} style={idx < activeChecklist.length - 1 ? [jn.checkRow, jn.checkRowBorder] : jn.checkRow}>
                {/* Checkbox */}
            ...
            <ImageBackground
              ...
              <View style={jn.halfCardOverlayWrap}>
                ...
                <View style={jn.halfCardOverlay}>
                  ...
                  <Text style={jn.halfCardTitleWhite}>My Groups</Text>
                  <Text style={jn.halfCardSubWhite}>Share milestones & updates</Text>
                </View>
              </View>
            </ImageBackground>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={jn.fullRowCard}
            ...
            <ImageBackground
              ...
              <View style={jn.halfCardOverlayWrap}>
                <View style={jn.halfCardCountWrap} />
                <View style={jn.halfCardOverlay}>
                  <Text style={jn.halfCardCategory}>CONTACTS</Text>
                  <Text style={jn.halfCardTitleWhite}>My Contacts</Text>
                  <Text style={jn.halfCardSubWhite}>Hotel, guide & emergency</Text>
                </View>
              </View>
            </ImageBackground>
          </TouchableOpacity>
      ...
      {/* Guide carousel — opens inline from this screen */}
      <GuideCarousel
...
const jn = StyleSheet.create({
  ...
  // ── 1. Board hero — 25% shorter (312 * 0.75 = 234) ─────────────────────────
  boardHero:        { borderRadius:20, overflow:"hidden", height:234, marginBottom:14, shadowColor:"#6A4A28", shadowOffset:{width:0,height:3}, shadowOpacity:0.14, shadowRadius:8, elevation:4 },
  boardHeroBg:      { flex:1 },
  boardHeroScrim:   { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.28)" },
  boardHeroContent: { position:"absolute", top:0, bottom:0, left:0, right:0, justifyContent:"flex-end", padding:20 },
  ...
  // ── 3. Guide card ────────────────────────────────────────────────────────────
  guideCard:        { borderRadius:20, overflow:"hidden", marginBottom:14, height:180, shadowColor:"#6A4A28", shadowOffset:{width:0,height:3}, shadowOpacity:0.14, shadowRadius:8, elevation:4 },
  guideCardBg:      { flex:1 },
  guideCardScrim:   { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.45)" },
  ...
  guideCardSub:     { fontSize:14, color:"rgba(255,255,255,0.75)", fontWeight:"400" },

  // ── 4. Full-width two-card row (same width as guide card) ────────────────────
  fullRow:          { flexDirection:"row", gap:12, marginBottom:14 },
  ...
  // Shared half-card overlay styles (used by fullRowCard too)
  ...
  // ── 5. Checklist dropdown ────────────────────────────────────────────────────
  checklistHeader:        { borderRadius:16, overflow:"hidden", marginBottom:4, height:96, shadowColor:"#6A4A28", shadowOffset:{width:0,height:3}, shadowOpacity:0.14, shadowRadius:8, elevation:4 },
  checklistHeaderBg:      { flex:1 },
  checklistHeaderScrim:   { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.48)", borderRadius:16 },
  ...
  checklistChev:          { fontSize:11, color:"rgba(255,255,255,0.8)" },

  checklistBody:   { backgroundColor:"#FDFAF4", borderRadius:14, borderWidth:1, borderColor:"#D4D0CA", overflow:"hidden", marginBottom:12, shadowColor:"#6A4A28", shadowOffset:{width:0,height:2}, shadowOpacity:0.08, shadowRadius:6, elevation:2 },
  ...
  compactArrow: { fontSize:22, color:"#D4D0CA" },

  // Legacy refs
  halfRow:      { flexDirection:"row", gap:12, marginBottom:14 },
