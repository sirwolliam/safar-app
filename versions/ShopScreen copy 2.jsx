  ...
  return (
    <TouchableOpacity style={[pt.tile, { width:tileW }]} onPress={onPress} activeOpacity={0.85}>
      <View style={pt.iconBox}>
        <item.Icon size={28} color={SAGE_L} weight="thin"/>
      </View>
...
const pt = StyleSheet.create({
  ...
  iconBox: {
    ...
    borderWidth:1, borderColor:"#D4DDD4",
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView showsVerticalScrollIndicator={false} bounces keyboardShouldPersistTaps="handled">
        ...
        {visibleProducts.length > 0 ? (
          <View style={s.grid}>
            {/* Build rows of 4 */}
            {Array.from({ length: Math.ceil(visibleProducts.length / 4) }).map((_, rowIdx) => (
              <View key={rowIdx} style={s.tileRow}>
                {visibleProducts.slice(rowIdx*4, rowIdx*4+4).map(item => (
                  <ProductTile
                    key={item.id}
                    item={item}
                    tileW={tileW}
                ...
                {visibleProducts.slice(rowIdx*4, rowIdx*4+4).length < 4 &&
                  Array.from({ length: 4 - visibleProducts.slice(rowIdx*4, rowIdx*4+4).length }).map((_,i) => (
