...
export default function HomeScreen({ navigation }) {
  ...
  const renderSlide = ({ item: slide }) => (
    <ImageBackground
      ...
      resizeMode="cover"
    >
      {/* Very light scrim on image only — keeps photo bright */}
      ...
      {/* Bottom panel — frosted dark glass holding all text + CTA */}
      <View style={s.heroPanel}>
        ...
        {slide.showGreeting ? (
          <TouchableOpacity activeOpacity={0.85} onPress={() => setShowAbout(true)}>
            <Text style={s.heroPanelGreeting}>{displayName}</Text>
          </TouchableOpacity>
        ) : null}
        {slide.headline ? (
          <Text style={s.heroPanelHeadline}>{slide.headline}</Text>
        ) : null}

        {/* Sub */}
        <Text style={s.heroPanelSub}>{slide.sub}</Text>

        {/* CTA — full-width solid green button like Global Healing */}
        <TouchableOpacity
        ...
        {/* Dot indicators — inside panel at bottom */}
...
const s = StyleSheet.create({
  ...
  heroPanel: {
    ...
    left:            0,
    right:           0,
    backgroundColor: "rgba(8,20,12,0.76)",
  ...
  // ── Hero: dot indicators (now inside panel) ───────────────────────────────
  heroDots: {
    flexDirection:  "row",
    justifyContent: "center",
    gap:            6,
