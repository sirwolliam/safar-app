 ...
 *
 * ONBOARDING WIRING:
 * On first launch (safar_onboarded_v1 not set), the app starts on the
 * Onboarding screen. OnboardingFlow calls navigation.replace("MainTabs")
 * on completion, which removes the onboarding from the stack permanently.
 */
...
export default function App() {
  ...
  // Wait for fonts AND async storage check before rendering
  if (!fontsLoaded || !onboardingChecked) return null;

  // To force the onboarding to show again during development:
  // AsyncStorage.removeItem("safar_onboarded_v1")
  // or add a Settings option that calls the same.
