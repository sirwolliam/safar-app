 ...
 */
import React, { useState, useEffect, useRef, useCallback, useCallback as uc } from "react";
import {
...
// ── Category badge row ────────────────────────────────────────────────────────
function BadgeRow({ cards, onScrollTo }) {
  const counts = {
    ...
    <View style={br.wrap}>
      {CARD_TYPES.map(t => (
        <TouchableOpacity key={t.key} style={[br.badge, { backgroundColor:t.tint, borderColor:t.pillBg }]}
          onPress={() => onScrollTo(t.key)} activeOpacity={0.8}>
...
function CategoryGroup({ type, cards, onDelete, onEdit, onToggle, onLayout }) {
  const cfg      = TYPE_MAP[type];
  const filtered = cards.filter(c => c.type === type);
  if (filtered.length === 0) return null;
  return (
    <View onLayout={onLayout} style={[cg.group, { borderColor:cfg.pillBg, backgroundColor:cfg.tint }]}>
...
export default function MyBoardScreen({ navigation }) {
  ...
  const scrollToType = (key) => {
    const y = groupRefs.current[key];
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {/* ── Category badge row — tap to scroll to section ── */}
      <BadgeRow cards={cards} onScrollTo={scrollToType} />
        ...
        <ScrollView ref={scrollRef} contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          {CARD_TYPES.map(t => (
            <CategoryGroup
              ...
              cards={cards}
              onDelete={deleteCard}
              onEdit={card => { setEditCard(card); setShowModal(true); }}
              onToggle={toggleCard}
              onLayout={e => { groupRefs.current[t.key] = e.nativeEvent.layout.y; }}
            />
          ))}
...
const s = StyleSheet.create({
  ...
  // Banner CTA — lighter green, centered, outlined + icon
  banner:          { flexDirection:"row", alignItems:"center", justifyContent:"center", backgroundColor:"#2F5D50", borderRadius:radius.lg, marginHorizontal:spacing(2.5), marginBottom:spacing(1.5), paddingVertical:spacing(1.75), paddingHorizontal:spacing(2), gap:spacing(1.25), ...shadows.card },
  bannerPlusCircle:{ width:26, height:26, borderRadius:13, borderWidth:1.5, borderColor:"rgba(255,255,255,0.8)", alignItems:"center", justifyContent:"center" },
  bannerPlus:      { fontSize:17, color:"#fff", lineHeight:22 },
  bannerTxt:       { fontFamily:SERIF, fontSize:18, color:"#fff", textAlign:"center" },
