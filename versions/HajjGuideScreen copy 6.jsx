 ...
 * Step-by-step Hajj guide with three action cards, checklist and carousel.
 */
...
// ── Three action cards shown under the hero ───────────────────────────────────
const ACTION_CARDS = [
  {
    id: "guide",
...
export default function HajjGuideScreen({ navigation }) {
  ...
  const handleCard = (action) => {
    if (action === "carousel") { setShowCarousel(true); return; }
    if (action === "duas")     { navigation?.navigate?.("DuaList", { list: { id:"umrah", name:"Hajj Du\u02bf\u0101\u02bes", count:23 } }); return; }
    if (action === "map")      { navigation?.navigate?.("Map"); return; }
  };
          ...
          >
            ...
            <View style={s.heroCardTop}>
              <View style={s.heroCardBadge}>
                <Text style={s.heroCardBadgeTxt}>HAJJ 101</Text>
              </View>
        ...
        <View style={s.cardRow}>
          {ACTION_CARDS.map((card) => (
            <TouchableOpacity
              ...
              style={s.actionCard}
              activeOpacity={0.88}
              onPress={() => handleCard(card.action)}
            >
              <ImageBackground
                source={card.image}
                style={{ flex:1 }}
                ...
                resizeMode="cover"
              >
                <View style={[s.actionScrim, { backgroundColor:card.scrim }]} />
                <View style={s.actionInner}>
                  <Text style={s.actionEyebrow}>{card.eyebrow}</Text>
...
const s = StyleSheet.create({
  ...
  heroCardScrim:   { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.48)" },
  ...
  heroCardBadgeTxt:{ fontFamily:SERIF, fontSize:13, color:colors.primary, fontWeight:"700" },
  heroCardArrow:   { fontSize:28, color:"rgba(255,255,255,0.7)" },
  ...
  // Three action cards
  cardRow:      { flexDirection:"row", gap:8, marginBottom:spacing(2.5) },
  actionCard:   { flex:1, height:110, borderRadius:12, overflow:"hidden", ...shadows.card },
  actionScrim:  { ...StyleSheet.absoluteFillObject, borderRadius:12 },
  actionInner:  { flex:1, justifyContent:"flex-end", padding:10 },
  actionEyebrow:{ fontSize:8, color:"rgba(255,255,255,0.65)", fontWeight:"700", textTransform:"uppercase", marginBottom:3 },
  actionTitle:  { fontFamily:SERIF, fontSize:15, color:"#FFFFFF", fontWeight:"600", lineHeight:19 },
