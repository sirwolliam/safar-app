...
const sc = (colors) => StyleSheet.create({
  card: {
    marginHorizontal: spacing(2.5), marginTop: -spacing(2.5),
    backgroundColor: colors.card, borderRadius: radius.lg, borderWidth: 1,
    borderColor: colors.border, padding: spacing(2.5), ...shadows.card,
  },
  handle: { width: 32, height: 3, borderRadius: 2, backgroundColor: colors.border, alignSelf: "center", marginBottom: spacing(2) },
  name: { fontFamily: SERIF, fontSize: typography.title, color: colors.text, marginBottom: 2 },
  arabic: { fontSize: typography.heading, color: colors.subtext, marginBottom: 2 },
  sub: { fontSize: typography.small, color: colors.subtext, marginBottom: spacing(1.25) },
  detail: { fontSize: typography.tiny, color: colors.subtext, lineHeight: 18, marginBottom: spacing(1.25), fontStyle: "italic" },
  countRow: {
    flexDirection: "row", justifyContent: "space-between", alignItems: "center",
    paddingTop: spacing(1.5), borderTopWidth: 1, borderTopColor: colors.border, marginBottom: spacing(2),
  },
  countLabel: { fontSize: typography.small, color: colors.subtext },
  countBadge: { backgroundColor: "#EBF2EE", paddingHorizontal: spacing(1.25), paddingVertical: 4, borderRadius: radius.pill },
  countValue: { fontSize: typography.small, color: colors.primary, fontWeight: "500" },
  btn: { backgroundColor: colors.primary, borderRadius: radius.md, paddingVertical: spacing(1.75), alignItems: "center", ...shadows.button },
  btnText: { fontSize: typography.body, color: "#fff", fontWeight: "500" },
});
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.cityToggle}>
        {["Makkah", "Madinah"].map((c) => (
          <TouchableOpacity
            key={c}
            style={[s.cityOpt, city === c && s.cityOptActive]}
            onPress={() => { setCity(c); setSelected(null); }}
            activeOpacity={0.85}
          >
            <Text style={[s.cityOptText, city === c && s.cityOptTextActive]}>{c}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.listSection}>
          ...
          {sites.map((site, i) => (
            <TouchableOpacity
              key={site.id}
              style={[s.listRow, i < sites.length - 1 && s.listRowBorder]}
              onPress={() => setSelected(site)}
              activeOpacity={0.85}
            >
              <View style={[s.dot, selected?.id === site.id && s.dotActive]}/>
              <View style={s.listInfo}>
                <Text style={[s.listName, selected?.id === site.id && s.listNameActive]}>{site.name}</Text>
                <Text style={s.listSub}>{site.sub}</Text>
              </View>
              <Text style={s.listCount}>{site.duas} duas</Text>
              <Text style={[s.listArrow, selected?.id === site.id && { color: colors.primary }]}>›</Text>
            </TouchableOpacity>
          ))}
        </View>
...
const createStyles = (colors) => StyleSheet.create({
  ...
  cityOptText: { fontSize: typography.body, color: colors.subtext },
  ...
  madinahNoteText: { fontSize: typography.tiny, color: "#7A6030", lineHeight: 18 },
  ...
  listInfo: { flex: 1 },
  listName: { fontFamily: SERIF, fontSize: typography.body, color: colors.text, marginBottom: 2 },
  listNameActive: { color: colors.primary },
  listSub: { fontSize: typography.tiny, color: colors.subtext },
  listCount: { fontSize: typography.small, color: colors.subtext },
  ...
  linksTitle: {
    fontFamily: SERIF, fontSize: typography.body, color: colors.text,
  ...
  linkText: { fontSize: typography.small, color: colors.primary, flex: 1 },
  linkArrow: { fontSize: typography.small, color: colors.primary },
});
