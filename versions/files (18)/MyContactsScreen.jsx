...
const CONTACT_COLORS = ["#2F5D50","#5B4A7A","#7A4A2F","#2F4A7A","#7A6B2F","#4A7A2F"];
...
const cc = StyleSheet.create({
  ...
  editBack:   { position:"absolute", left:0, top:0, bottom:0, width:80, backgroundColor:"#2F5D50", borderRadius:14, alignItems:"center", justifyContent:"center" },
  ...
  card:         { borderRadius:14, borderWidth:1.5, overflow:"hidden", shadowColor:"#1A1712", shadowOffset:{width:0,height:2}, shadowOpacity:0.08, shadowRadius:6, elevation:3 },
  ...
  mainInfo:     { flex:1 },
  name:         { fontFamily:SERIF, fontSize:18, fontWeight:"400", marginBottom:3, lineHeight:22 },
  phone:        { fontSize:15, color:"#1A1712", fontWeight:"400" },
  phonePlaceholder: { fontSize:13, color:"#4A4846", fontStyle:"italic" },

  rightCol:     { alignItems:"flex-end", gap:6 },
  starBtn:      { },
  starIcon:     { fontSize:20, color:"#D4D0CA" },
  ...
  expandInfo:   { flex:1 },
  expandLabel:  { fontSize:11, color:"#4A4846", fontWeight:"500", marginBottom:1 },
  expandValue:  { fontSize:15, fontWeight:"400" },
  expandNote:   { fontSize:14, color:"#3D3935", lineHeight:20 },
  expandArrow:  { fontSize:18, color:"#D4D0CA" },
  noDetails:    { fontSize:13, color:"#4A4846", fontStyle:"italic", paddingVertical:10, textAlign:"center" },
        ...
        <View style={cm.overlay}>
          <View style={cm.sheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <TextInput style={cm.input} placeholder="e.g. Al-Andalus Hotel" placeholderTextColor="#7A7670" value={name} onChangeText={setName} autoFocus />
              ...
              <TextInput style={cm.input} placeholder="+966 XX XXX XXXX" placeholderTextColor="#7A7670" value={phone} onChangeText={setPhone} keyboardType="phone-pad" />

              <Text style={cm.fieldLabel}>WHATSAPP NUMBER</Text>
              <TextInput style={cm.input} placeholder="+966 XX XXX XXXX (if different)" placeholderTextColor="#7A7670" value={whatsapp} onChangeText={setWhatsapp} keyboardType="phone-pad" />

              <Text style={cm.fieldLabel}>EMAIL</Text>
              <TextInput style={cm.input} placeholder="contact@example.com" placeholderTextColor="#7A7670" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />

              <Text style={cm.fieldLabel}>NOTE</Text>
              <TextInput style={[cm.input, cm.inputMulti]} placeholder="e.g. Check-in 3pm, ask for Ibrahim at reception" placeholderTextColor="#7A7670" value={note} onChangeText={setNote} multiline />
...
const cm = StyleSheet.create({
  ...
  sheet:         { backgroundColor:"#D4E4DC", borderTopLeftRadius:24, borderTopRightRadius:24, paddingHorizontal:20, paddingBottom:16, maxHeight:"90%" },
  ...
  swatchActive:  { borderWidth:2.5, borderColor:"#1A1712" },
  ...
  input:         { backgroundColor:"#FDFAF4", borderRadius:10, borderWidth:1, borderColor:"#D4D0CA", paddingHorizontal:14, paddingVertical:12, fontSize:16, color:"#1A1712", marginBottom:14, shadowColor:"#6A4A28", shadowOffset:{width:0,height:2}, shadowOpacity:0.08, shadowRadius:4, elevation:2 },
  inputMulti:    { minHeight:72, textAlignVertical:"top" },
  roleRow:       { gap:8, paddingBottom:14 },
  roleChip:      { paddingHorizontal:14, paddingVertical:8, borderRadius:999, borderWidth:1, borderColor:"#D4D0CA", backgroundColor:"#FDFAF4" },
  roleChipOn:    { backgroundColor:"#2F5D50", borderColor:"#2F5D50" },
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.searchWrap}>
        ...
        <TouchableOpacity style={s.importRow} onPress={() => setShowImportTip(v => !v)} activeOpacity={0.8}>
          <Text style={s.importIcon}>{"📲"}</Text>
          <Text style={s.importTxt}>Import from phone contacts</Text>
          <Text style={s.importChev}>{showImportTip ? "▲" : "▼"}</Text>
        </TouchableOpacity>
        {showImportTip && (
      ...
      {contacts.some(c => c.starred) && !searchQuery && (
        <View style={s.sectionLabel}>
          <Text style={s.sectionLabelIcon}>{"★"}</Text>
...
const s = StyleSheet.create({
  ...
  header:      { flexDirection:"row", alignItems:"center", justifyContent:"space-between", paddingHorizontal:20, paddingTop:16, paddingBottom:14, backgroundColor:"#EDE6D8" },
  ...
  headerTitle: { fontFamily:SERIF, fontSize:22, color:"#1A1712" },
  headerSub:   { fontSize:12, color:"#3D3935", marginTop:1 },
  addBtn:      { width:36, height:36, borderRadius:18, backgroundColor:"#2F5D50", alignItems:"center", justifyContent:"center", shadowColor:"#6A4A28", shadowOffset:{width:0,height:3}, shadowOpacity:0.14, shadowRadius:8, elevation:4 },
  ...
  searchBar:    { flexDirection:"row", alignItems:"center", gap:10, backgroundColor:"#FDFAF4", borderRadius:14, borderWidth:1.5, borderColor:"#D4D0CA", paddingHorizontal:14, paddingVertical:11, shadowColor:"#6A4A28", shadowOffset:{width:0,height:2}, shadowOpacity:0.08, shadowRadius:4, elevation:2 },
  searchFocused:{ borderColor:"#2F5D50" },
  searchIcon:   { fontSize:15, opacity:0.5 },
  searchInput:  { flex:1, fontSize:16, color:"#1A1712", padding:0 },
  searchClear:  { fontSize:14, color:"#6B6259" },

  // Import row
  importRow:      { flexDirection:"row", alignItems:"center", gap:8, paddingVertical:10, paddingHorizontal:4 },
  importIcon:     { fontSize:16 },
  importTxt:      { flex:1, fontSize:14, color:"#2F5D50", fontWeight:"500" },
  importChev:     { fontSize:11, color:"#3D3935" },
  importTipPanel: { backgroundColor:"#F5EDD8", borderRadius:12, borderWidth:1, borderColor:"#E8D9B8", padding:14, marginTop:2 },
  importTipText:  { fontSize:13, color:"#7A6030", lineHeight:20 },

  // Section label (VIP)
  sectionLabel:    { flexDirection:"row", alignItems:"center", gap:6, paddingHorizontal:20, paddingBottom:8 },
  sectionLabelIcon:{ fontSize:14, color:"#C8A96A" },
  sectionLabelTxt: { fontSize:11, fontWeight:"700", letterSpacing:1.5, color:"#5C4F3A", textTransform:"uppercase" },

  empty:       { flex:1, alignItems:"center", justifyContent:"center", padding:32 },
  emptyEmoji:  { fontSize:44, marginBottom:16 },
  emptyTitle:  { fontFamily:SERIF, fontSize:20, color:"#1A1712", marginBottom:8 },
  emptyBody:   { fontSize:16, color:"#3D3935", textAlign:"center", lineHeight:22, marginBottom:20 },
  emptyBtn:    { backgroundColor:"#2F5D50", borderRadius:10, paddingVertical:12, paddingHorizontal:24, shadowColor:"#6A4A28", shadowOffset:{width:0,height:3}, shadowOpacity:0.14, shadowRadius:8, elevation:4 },
