      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        {CURRENCIES.map((currency, i) => {
          ...
          return (
            <TouchableOpacity
              key={currency.code}
              style={[s.row, isSelected && s.rowSelected, i === CURRENCIES.length - 1 && s.rowLast]}
              onPress={() => setSelected(isSelected ? null : currency.code)}
              activeOpacity={0.85}
            >
              <Text style={s.flag}>{currency.flag}</Text>
              <View style={s.currencyInfo}>
                <Text style={[s.code, isSelected && s.codeSelected]}>{currency.code}</Text>
        ...
        <View style={s.quickSection}>
          ...
          <View style={s.quickRow}>
            {[10, 50, 100, 500].map((amt) => (
              <TouchableOpacity
                key={amt}
                style={[s.quickBtn, sarAmount === String(amt) && s.quickBtnActive]}
                onPress={() => { setSarAmount(String(amt)); setSelected(null); setForeignAmount(""); }}
              >
                <Text style={[s.quickBtnText, sarAmount === String(amt) && s.quickBtnTextActive]}>
                  {amt} SAR
                </Text>
              </TouchableOpacity>
            ))}
...
const createStyles = (colors) => StyleSheet.create({
  ...
  sarCard: {
    ...
    padding: spacing(2.5), flexDirection: "row", alignItems: "center",
    justifyContent: "space-between", marginBottom: spacing(1), ...shadows.button,
  },
  sarLeft: { flexDirection: "row", alignItems: "center", gap: spacing(1.25) },
  sarFlag: { fontSize: 32 },
  sarCode: { fontFamily: SERIF, fontSize: typography.heading, color: "#fff" },
  sarName: { fontSize: typography.tiny, color: "rgba(255,255,255,0.7)", marginTop: 2 },
  sarInputWrap: {
  ...
  statusOffline: { fontSize: typography.tiny, color: "#7A6030", fontStyle: "italic" },
  statusOnline:  { fontSize: typography.tiny, color: colors.subtext },
  refreshText:   { fontSize: typography.tiny, color: colors.primary, fontWeight: "500" },

  scroll: { paddingHorizontal: spacing(2.5) },

  row: {
    flexDirection: "row", alignItems: "center", gap: spacing(1.5),
    paddingVertical: spacing(1.75), borderBottomWidth: 1, borderBottomColor: colors.border,
  },
  rowSelected: {
    backgroundColor: "#EBF2EE", marginHorizontal: -spacing(2.5),
    paddingHorizontal: spacing(2.5), borderRadius: radius.sm,
    borderBottomColor: "transparent",
  },
  rowLast: { borderBottomWidth: 0 },
  flag: { fontSize: 28, width: 36, textAlign: "center" },
  currencyInfo: { flex: 1 },
  code: { fontSize: typography.body, fontWeight: "600", color: colors.text },
  codeSelected: { color: colors.primary },
  currencyName: { fontSize: typography.tiny, color: colors.subtext, marginTop: 2 },
  amountWrap: { alignItems: "flex-end", minWidth: 100 },
  amount: { fontFamily: SERIF, fontSize: typography.heading, color: colors.text },
  rateHint: { fontSize: 10, color: colors.subtext, marginTop: 2 },
  foreignInput: {
    fontFamily: SERIF, fontSize: typography.heading, color: colors.primary,
    textAlign: "right", minWidth: 100,
    borderBottomWidth: 1.5, borderBottomColor: colors.primary,
    paddingBottom: 2,
  },

  quickSection: { marginTop: spacing(2.5) },
  quickTitle: {
    fontSize: typography.tiny, fontWeight: "600", color: colors.subtext,
    letterSpacing: 1, marginBottom: spacing(1),
  },
  quickRow: { flexDirection: "row", gap: spacing(1) },
  quickBtn: {
    flex: 1, paddingVertical: spacing(1.25), borderRadius: radius.md,
    borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card,
    alignItems: "center", ...shadows.card,
  },
  quickBtnActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  quickBtnText: { fontSize: typography.small, color: colors.subtext },
  quickBtnTextActive: { color: "#fff", fontWeight: "500" },

  disclaimer: {
    fontSize: typography.tiny, color: colors.subtext, fontStyle: "italic",
    textAlign: "center", marginTop: spacing(2), lineHeight: 18,
  },
});
