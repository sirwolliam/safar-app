      ...
      <Animated.View
        ...
        <View style={s.msFooter}>
          <TouchableOpacity
            ...
            {busy
              ? <ActivityIndicator size="small" color={hasAmeen ? "#fff" : T"#1E3D30"}/>
...
export default function GroupsScreen({ navigation }) {
  ...
  const accent     = GROUP_COLORS.find(g => g.key===(activeG?.colorKey??"green"))?.value ?? T"#1E3D30";
      ...
      <View style={s.tabsWrap}>
        <FlatList horizontal data={allGroups} keyExtractor={g=>g.id}
          ...
          renderItem={({item:g}) => {
            ...
            const ac = GROUP_COLORS.find(x=>x.key===(g.colorKey??"green"))?.value ?? T"#1E3D30";
            return (
              <TouchableOpacity
          ...
          <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => { setShowPost(false); setPostText(""); setPostLink(""); setShowLink(false); }}>
            <View style={s.sheet} onStartShouldSetResponder={() => true}>
              ...
              <TextInput style={s.mInput} placeholder={"e.g. Completed Tawaf \uD83D\uDD4B"}
                placeholderTextColor={T"#5C534A"} value={postText}
              ...
              {showLink ? (
                <View style={s.linkBox}>
                  <TextInput style={s.linkIn} placeholder="https://..." placeholderTextColor={T"#5C534A"}
                    value={postLink} onChangeText={setPostLink} keyboardType="url" autoCapitalize="none" autoCorrect={false}/>
                  <TextInput style={s.linkIn} placeholder="Link title (optional)" placeholderTextColor={T"#5C534A"}
        ...
        <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==="ios"?"padding":"height"}>
          <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => { setShowNew(false); setNewName(""); }}>
            <View style={s.sheet} onStartShouldSetResponder={() => true}>
              ...
              <TextInput style={s.mInputSm} placeholder="e.g. Family Hajj 2026" placeholderTextColor={T"#5C534A"}
        ...
        <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==="ios"?"padding":"height"}>
          <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => setShowEdit(false)}>
            <View style={s.sheet} onStartShouldSetResponder={() => true}>
              ...
              <TextInput style={s.mInputSm} placeholder="Group name" placeholderTextColor={T"#5C534A"}
          ...
          <View style={s.sheet} onStartShouldSetResponder={() => true}>
            ...
            {allGroups.filter(g=>g.id!==activeId).map(g => (
              <TouchableOpacity key={g.id} style={s.optRow}
                ...
                <View style={s.optIcon}><GroupIcon size={16} color={T"#1E3D30"}/></View>
