...
function HeaderPattern({ width, height }) {
  return (
    <Svg
      width={width}
      height={height}
      viewBox={HEADER_PATTERN_VIEWBOX}
...
const ctr = StyleSheet.create({
  ...
  txt:  { fontSize:13, color:MUTED, fontWeight:"400" },
});
  ...
  return (
    <>
      <SafeAreaView style={s.safe}>
        ...
        {/* ── Top nav: back · bookmark — sits above header zone ── */}
        <View style={s.topNav}>
        ...
        {/* ── Header zone: pattern + arch + star (overlapping nav) + stage + title ── */}
        <View
          style={s.headerZone}
          onLayout={e => setHeaderH(e.nativeEvent.layout.height)}
        >
          {/* Geometric pattern behind everything */}
          <HeaderPattern width={SW} height={headerH} />

          {/* Subtle pointed arch framing the title */}
          <ArchFrame width={SW} />

          {/* Star — absolutely positioned to vertically overlap with nav buttons above */}
        ...
        {/* ── Fixed controls bar — dark sage green ── */}
        <View style={s.ctrlBar}>
          ...
          <View style={s.ctrlRow}>
            ...
            {/* Previous */}
            <TouchableOpacity style={s.ctrlItem} onPress={goPrev} activeOpacity={hasPrev ? 0.75 : 1}>
              <View style={s.ctrlCircle}>
                <IconArrowLeft dim={!hasPrev} size={20} />
              </View>
              <Text style={s.ctrlLabel}>Previous</Text>
            </TouchableOpacity>

            {/* Repeat / Loop */}
            <TouchableOpacity style={s.ctrlItem} onPress={() => setLooping(v => !v)} activeOpacity={0.8}>
              <View style={[s.ctrlCircle, looping ? s.ctrlCircleOn : null]}>
                <IconRepeat size={20} color={looping ? GOLD : CTRL_ICON} />
              </View>
              <Text style={[s.ctrlLabel, looping ? s.ctrlLabelOn : null]}>Repeat</Text>
            </TouchableOpacity>

            {/* Play / Pause — large white circle */}
            <TouchableOpacity style={s.ctrlItem} onPress={audio.toggle} activeOpacity={0.88}>
            ...
            {/* Next */}
            <TouchableOpacity style={s.ctrlItem} onPress={goNext} activeOpacity={hasNext ? 0.75 : 1}>
              <View style={s.ctrlCircle}>
                <IconArrowRight dim={!hasNext} size={20} />
              </View>
              <Text style={s.ctrlLabel}>Next</Text>
            </TouchableOpacity>

            {/* Speed */}
            <TouchableOpacity style={s.ctrlItem} onPress={() => setShowSpeed(v => !v)} activeOpacity={0.8}>
              <View style={s.ctrlCircle}>
          ...
          {/* Toggles row: Transliteration [toggle] | Translation [toggle] */}
...
const s = StyleSheet.create({
  ...
  safe: { flex:1, backgroundColor:CTRL_BG },

  safe: { flex:1, backgroundColor:BG },

  // Top nav — above header zone, same row height as before
  topNav: {
    flexDirection:"row",
  ...
  // Header zone — starts immediately after nav, no extra top padding
  headerZone: {
    alignItems:"center",
    paddingBottom:spacing(2),
    overflow:"hidden",
    backgroundColor:BG,
    // Pull up by nav height so star appears at same vertical level as nav buttons
    marginTop:-44,
    paddingTop:44,
  },

  // Star — centered, sits at the top of headerZone, visually beside nav buttons
  headerStar: {
    marginTop:10,
    marginBottom:5,
    zIndex:2,
