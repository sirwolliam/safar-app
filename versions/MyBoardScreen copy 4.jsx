...
function StatsRow({ cards }) {
  ...
  const duas      = cards.filter(c => c.type==="dua").length;

  return (
    <View style={st.row}>
      <View style={st.stat}>
        <Text style={st.num}>{total}</Text>
        <Text style={st.lbl}>cards</Text>
      </View>
      <View style={st.divider} />
      <View style={st.stat}>
        <Text style={st.num}>{notes}</Text>
        <Text style={st.lbl}>notes</Text>
      </View>
      <View style={st.divider} />
      <View style={st.stat}>
        <Text style={st.num}>{duas}</Text>
        <Text style={st.lbl}>du\u02bf\u0101\u02bes</Text>
      </View>
      {checklist > 0 ? (
        <>
          <View style={st.divider} />
          <View style={st.stat}>
            <Text style={[st.num, { color:colors.primary }]}>{done}/{checklist}</Text>
            <Text style={st.lbl}>tasks</Text>
          </View>
          <View style={st.progWrap}>
            <View style={[st.progFill, { width:((done/checklist)*100)+"%" }]} />
          </View>
        </>
      ) : null}
    </View>
...
const st = StyleSheet.create({
  row:      { flexDirection:"row", alignItems:"center", backgroundColor:colors.card, borderRadius:radius.md, borderWidth:1, borderColor:colors.border, marginHorizontal:spacing(2.5), marginBottom:spacing(1.5), paddingVertical:spacing(1.25), paddingHorizontal:spacing(2), gap:spacing(1.5), ...shadows.card },
  stat:     { alignItems:"center" },
  num:      { fontFamily:SERIF, fontSize:18, color:colors.text, lineHeight:22 },
  lbl:      { fontSize:10, color:colors.subtext, letterSpacing:0.3 },
  divider:  { width:1, height:28, backgroundColor:colors.border },
  progWrap: { flex:1, height:4, backgroundColor:colors.border, borderRadius:2, overflow:"hidden" },
...
export default function MyBoardScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {/* ── Hero add bar — primary CTA ── */}
      <TouchableOpacity style={s.addBar} onPress={openAdd} activeOpacity={0.88}>
        <View style={s.addBarIcon}><Text style={s.addBarPlus}>+</Text></View>
        <Text style={s.addBarTxt}>Add a note, task, du\u02bf\u0101\u02be or link</Text>
        <Text style={s.addBarArrow}>{"\u203a"}</Text>
      </TouchableOpacity>

      {/* ── Stats row (only when there are cards) ── */}
      {cards.length > 0 ? <StatsRow cards={cards} /> : null}
      ...
      {cards.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyBody}>This is your personal space. Save your intentions, packing list, pinned du\u02bf\u0101\u02bes and travel links — all in one place.</Text>
          <View style={s.emptyTypeRow}>
            {CARD_TYPES.map(t => (
              <View key={t.key} style={[s.emptyTypePill, { backgroundColor:t.tint, borderColor:t.pillBg }]}>
                <Text style={s.emptyTypePillTxt}>{t.emoji} {t.label}</Text>
              </View>
            ))}
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  // Header
  header:       { flexDirection:"row", alignItems:"center", paddingHorizontal:spacing(2.5), paddingTop:spacing(2), paddingBottom:spacing(1.5) },
  ...
  // Hero add bar
  addBar:       { flexDirection:"row", alignItems:"center", backgroundColor:colors.primary, borderRadius:radius.lg, marginHorizontal:spacing(2.5), marginBottom:spacing(1.5), paddingVertical:spacing(1.75), paddingHorizontal:spacing(2), gap:spacing(1.25), ...shadows.button },
  addBarIcon:   { width:28, height:28, borderRadius:14, backgroundColor:"rgba(255,255,255,0.2)", alignItems:"center", justifyContent:"center" },
  addBarPlus:   { fontSize:20, color:"#fff", lineHeight:24 },
  addBarTxt:    { flex:1, fontFamily:SERIF, fontSize:15, color:"#fff" },
  addBarArrow:  { fontSize:20, color:"rgba(255,255,255,0.7)" },

  // Card list
  scroll:    { paddingHorizontal:spacing(2.5), paddingTop:spacing(0.5) },
  swipeHint: { fontSize:typography.tiny, color:colors.subtext, textAlign:"center", paddingTop:spacing(1), paddingBottom:spacing(1) },
  ...
  emptyBody:      { fontSize:typography.body, color:colors.subtext, textAlign:"center", lineHeight:22, marginBottom:spacing(2) },
  emptyTypeRow:   { flexDirection:"row", flexWrap:"wrap", gap:spacing(0.75), justifyContent:"center" },
  emptyTypePill:  { borderRadius:radius.pill, borderWidth:1, paddingHorizontal:spacing(1.25), paddingVertical:spacing(0.625) },
  emptyTypePillTxt:{ fontSize:12, color:colors.text },
});
