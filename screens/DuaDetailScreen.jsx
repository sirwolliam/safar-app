...
// ── Header pattern — SVG with built-in gradient mask ─────────────────────────
// Pattern fades from fully visible at top to fully transparent at bottom.
// Renders seamlessly into the parchment background with no hard edge.
function HeaderPattern({ width, height }) {
  // Scale the 375×133.62 viewBox to fill actual screen width
  ...
  return (
    ...
    >
      <Defs>
        {/* Gradient: white at top = fully visible, black at bottom = hidden */}
      ...
      <View style={[s.safe, { paddingBottom: 0 }]}>
        ...
        <View style={[s.ctrlBar, { paddingBottom: spacing(2) + insets.bottom }]}>
          ...
          <View style={s.ctrlRow}>
            ...
            {/* Play / Pause — moved up 10px using position relative */}
            <TouchableOpacity style={[s.ctrlItem, { position:"relative", top:-10 }]} onPress={audio.toggle} activeOpacity={0.88}>
...
const s = StyleSheet.create({
  ...
  ctrlItem: {
    alignItems:"center",
    gap:6,
    minWidth:56,   // guaranteed hit area wider than circle
  },
  ctrlCircle: {
    width:40,
    height:40,
