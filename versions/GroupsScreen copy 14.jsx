 ...
 * Single StyleSheet inside useMemo. Zero module-level style references.
 * All conditional styles use ternary (not &&) to avoid false in style arrays.
 */
...
const INIT_MILESTONES = {
  ex1:[
    { id:"m1", author:"Ahmed Al-Rashid", uid:"u2", text:"Completed Tawaf al-Qudum, alhamdulillah \uD83D\uDD4B", time:"2h ago", ameen:["u3","u4"], count:2, link:null, linkTitle:null },
    ...
    { id:"m3", author:"Maryam Khan",     uid:"u3", text:"First time seeing the Kaaba. Subhanallah.", time:"6h ago", ameen:["u1","u2","u4"], count:3, link:null, linkTitle:null },
  ],
  ex2:[
    { id:"m4", author:"Zainab Ali",   uid:"u5", text:"Completed Sa\u02bfy \u2014 alhamdulillah \uD83C\uDF3F", time:"1h ago", ameen:["u3","u4","u6"], count:3, link:null, linkTitle:null },
    { id:"m5", author:"Noor Ibrahim", uid:"u6", text:"Drank Zamzam water for the first time \uD83D\uDCA7", time:"3h ago", ameen:["u5"], count:1, link:null, linkTitle:null },
  ],
  ex3:[
    { id:"m6", author:"Tariq Hassan", uid:"u7", text:"Arrived in Makkah safely. Ya Allah \uD83D\uDD4B", time:"5h ago", ameen:["u2","u4","u8"], count:3, link:null, linkTitle:null },
...
function MilestoneRow({ item, myUid, onAmeen, onDelete, s }) {
  ...
  const pan = useRef(PanResponder.create({
    ...
    onPanResponderMove:    (_,gs) => tx.setValue(Math.max(-95, Math.min(0, gs.dx))),
    onPanResponderRelease: (_,gs) => {
      Animated.spring(tx, { toValue: gs.dx < -55 ? -80 : 0, useNativeDriver:true }).start();
    },
  })).current;
  ...
  return (
    <View style={s.msWrap}>
      ...
      <Animated.View
        style={[s.msCard, { backgroundColor:uc+"18", borderColor:uc+"30" }, { transform:[{translateX:tx}] }]}
        {...pan.panHandlers}>
        <View style={s.msTop}>
        ...
        <View style={s.msFooter}>
          <TouchableOpacity
            ...
            {busy
              ...
              : <><Text>{"\uD83E\uDD32"}</Text><Text style={hasAmeen ? s.msBtnTxtOn : s.msBtnTxt}>{hasAmeen ? "Āmeen" : "Say Āmeen"}</Text></>
            }
          </TouchableOpacity>
...
export default function GroupsScreen({ navigation }) {
  ...
  const s = useMemo(() => StyleSheet.create({
    ...
    newBtnTxt:   { fontSize:typography.small, color:"#fff", fontWeight:"600" },
    ...
    tabName:     { fontFamily:SERIF, fontSize:typography.small, color:C.text, marginBottom:1 },
    tabCount:    { fontSize:typography.tiny, color:C.subtext },
    ...
    bannerTxt:   { fontSize:typography.small, color:"#7A6030", marginBottom:spacing(0.5) },
    bannerCta:   { fontSize:typography.small, color:C.primary, fontWeight:"500" },
    ...
    membersTtl:  { fontFamily:SERIF, fontSize:typography.small, fontWeight:"600" },
    editGrpBtn:  { fontSize:typography.small, fontWeight:"500" },
    memberRow:   { gap:spacing(1.5), paddingBottom:spacing(0.5) },
    memberWrap:  { alignItems:"center", gap:4, width:60 },
    memberName:  { fontSize:typography.tiny, color:C.text, textAlign:"center" },
    memberHint:  { fontSize:8, color:C.border },
    ...
    feedTitle:   { fontFamily:SERIF, fontSize:typography.heading, color:C.text },
    shareBtn:    { borderRadius:radius.pill, paddingHorizontal:spacing(1.75), paddingVertical:spacing(0.875) },
    shareBtnTxt: { fontSize:typography.small, color:"#fff", fontWeight:"600" },
    empty:       { alignItems:"center", paddingVertical:spacing(4) },
    emptyEmoji:  { fontSize:36, marginBottom:spacing(1.5) },
    emptyTxt:    { fontSize:typography.body, color:C.subtext, textAlign:"center", lineHeight:22 },
    // milestone
    msWrap:      { marginBottom:spacing(1.25), borderRadius:radius.md, overflow:"hidden" },
    msBack:      { position:"absolute", right:0, top:0, bottom:0, width:80, backgroundColor:"#E05252", borderRadius:radius.md, alignItems:"center", justifyContent:"center" },
    msBackBtn:   { alignItems:"center", gap:4, width:"100%", height:"100%", justifyContent:"center" },
    msBackTxt:   { fontSize:typography.tiny, color:"#fff", fontWeight:"600" },
    ...
    msName:      { fontFamily:SERIF, fontSize:typography.small, fontWeight:"500", color:C.text },
    msTime:      { fontSize:typography.tiny, color:C.subtext },
    msText:      { fontSize:typography.body, color:C.text, lineHeight:22, marginBottom:spacing(1.25) },
    ...
    msLinkTxt:   { flex:1, fontSize:typography.tiny, color:C.primary, fontWeight:"400" },
    msLinkArrow: { fontSize:12, color:C.primary },
    msFooter:    { flexDirection:"row", alignItems:"center", justifyContent:"space-between" },
    msBtn:       { flexDirection:"row", alignItems:"center", gap:spacing(0.75), paddingHorizontal:spacing(1.5), paddingVertical:spacing(0.75), borderRadius:radius.pill, borderWidth:1, borderColor:C.border, backgroundColor:"rgba(255,255,255,0.5)" },
    msBtnOn:     { flexDirection:"row", alignItems:"center", gap:spacing(0.75), paddingHorizontal:spacing(1.5), paddingVertical:spacing(0.75), borderRadius:radius.pill, borderWidth:1, borderColor:C.primary, backgroundColor:C.primary },
    msBtnTxt:    { fontSize:typography.small, color:C.primary },
    msBtnTxtOn:  { fontSize:typography.small, color:"#fff", fontWeight:"500" },
    msCount:     { fontSize:typography.tiny, color:C.subtext },
    // modals
    ...
    mSub:        { fontSize:typography.small, color:C.subtext, marginBottom:spacing(1.5) },
    mInput:      { backgroundColor:C.card, borderRadius:radius.md, borderWidth:1, borderColor:C.border, padding:spacing(1.75), fontSize:typography.body, color:C.text, minHeight:88, textAlignVertical:"top", marginBottom:spacing(0.5) },
    mInputSm:    { backgroundColor:C.card, borderRadius:radius.md, borderWidth:1, borderColor:C.border, padding:spacing(1.75), fontSize:typography.body, color:C.text, minHeight:52, marginBottom:spacing(0.5) },
    mCharCount:  { fontSize:typography.tiny, color:C.subtext, textAlign:"right", marginBottom:spacing(1.25) },
    attachRow:   { flexDirection:"row", alignItems:"center", gap:spacing(1), marginBottom:spacing(1) },
    attachLbl:   { fontSize:typography.tiny, color:C.subtext },
    ...
    attachTxt:   { fontSize:typography.small, color:C.subtext },
    attachTxtOn: { fontSize:typography.small, color:C.primary, fontWeight:"600" },
    linkBox:     { gap:spacing(0.75), marginBottom:spacing(1) },
    linkIn:      { backgroundColor:C.card, borderRadius:radius.sm, borderWidth:1, borderColor:C.border, paddingHorizontal:spacing(1.5), paddingVertical:spacing(1), fontSize:typography.small, color:C.text },
    ...
    cancelTxt:   { fontSize:typography.body, color:C.text },
    ...
    submitTxt:   { fontSize:typography.body, color:"#fff", fontWeight:"600" },
    delBtn:      { marginTop:spacing(2), alignItems:"center", paddingVertical:spacing(1.5) },
    delTxt:      { fontSize:typography.body, color:"#E05252", fontWeight:"500" },
    // member sheet
    mshRow:      { flexDirection:"row", alignItems:"center", gap:spacing(1.5), marginBottom:spacing(2), marginTop:spacing(0.5) },
    mshName:     { fontFamily:SERIF, fontSize:typography.heading, color:C.text },
    mshSub:      { fontSize:typography.small, color:C.subtext },
    ...
    optTxt:      { flex:1, fontFamily:SERIF, fontSize:typography.body, color:C.text },
    optPlus:     { fontSize:20, color:C.primary },
    removeRow:   { paddingVertical:spacing(2), alignItems:"center" },
    removeTxt:   { fontSize:typography.body, color:"#E05252", fontWeight:"500" },
    cancelRow:   { paddingVertical:spacing(1.5), alignItems:"center" },
    cancelRowTxt:{ fontSize:typography.body, color:C.subtext },
  }), [C]);
  ...
  const doPost = async () => {
    ...
    if (isEx) {
      setExMilestones(p => ({ ...p, [activeId]:[{ id:`m${Date.now()}`,author:"You",uid:myUid,text:postText.trim(),time:"just now",ameen:[],count:0,link:lnk,linkTitle:postLinkTitle.trim()||lnk }, ...(p[activeId]??[])] }));
    } else {
  ...
  const doCreate = async () => {
    ...
    if (usingEx) {
      const g = { id:`local_${Date.now()}`, name:newName.trim(), memberUids:["u4"], isExample:true, colorKey:newColor };
      setExGroups(p => [...p, g]);
      setExMilestones(p => ({ ...p, [g.id]:[] }));
      setActiveId(g.id);
    } else {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.membersBox, {borderColor:accent+"30"}]}>
        ...
        <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==="ios"?"padding":"height"}>
          <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => { setShowPost(false); setPostText(""); setPostLink(""); setShowLink(false); }}>
            <View style={s.sheet} onStartShouldSetResponder={() => true}>
              ...
              <TextInput style={s.mInput} placeholder={"e.g. Completed Tawaf \uD83D\uDD4B"} placeholderTextColor={TC.subtext}
                value={postText} onChangeText={t => setPostText(t.slice(0,MAX_CHARS))} multiline maxLength={MAX_CHARS} autoFocus />
              ...
              <View style={s.btnRow}>
                ...
                <TouchableOpacity style={(!postText.trim()||posting) ? [s.submitBtn, s.submitDim] : s.submitBtn}
                  onPress={doPost} disabled={posting||!postText.trim()}>
