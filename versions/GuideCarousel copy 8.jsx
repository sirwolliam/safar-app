...
function Slide({ step, total, guideTitle, isActive }) {
  // Ken Burns zoom — slow scale from 1.0 to 1.08
  const scaleAnim = useRef(new Animated.Value(1.0)).current;
  // Divider width reveal — 0 to 28
  const lineAnim  = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    if (isActive) {
      // Reset and start zoom
...
const gc = StyleSheet.create({
  ...
  // Arrows at 1/3 screen height
  arrow:        { position:"absolute", top: Math.round(SH * 0.33), marginTop:-22 },
  arrowLeft:    { left:16 },
  arrowRight:   { right:16 },
