    ...
    <View style={st.msWrap}>
      ...
      <Animated.View style={[st.msCard,{backgroundColor:tintBg,borderColor:userColor+"30",transform:[{translateX:tx}]}]}
        ...
        <View style={st.msFooter}>
          <TouchableOpacity style={[st.msBtn,hasAmeen ? st.msBtnActive : null]}
            ...
            {busy
              ...
              : <><Text>{"\uD83E\uDD32"}</Text><Text style={[st.msBtnText,hasAmeen ? st.msBtnTextActive : null]}>{hasAmeen?"Āmeen":"Say Āmeen"}</Text></>
            }
          </TouchableOpacity>
...
export default function GroupsScreen({ navigation }) {
  const { colors: _colors } = useAccessibility();
      ...
      <View style={st.tabsWrap}>
        <FlatList horizontal data={allGroups} keyExtractor={g=>g.id}
          ...
          renderItem={({item:g}) => {
            ...
            return (
              <TouchableOpacity style={[st.tab, active ? {borderColor:ac, backgroundColor:ac+"12"} : null]}
                onPress={()=>setActiveId(g.id)} activeOpacity={0.85}>
                <View style={st.tabIconRow}>
                  <View style={[st.tabIconWrap, active ? {backgroundColor:ac} : null]}>
                    <GroupIcon size={16} color={active?"#fff":ac}/>
                  </View>
                  <View>
                    <Text style={[st.tabName, active ? {color:ac} : null]}>{g.name}</Text>
            ...
            <View style={st.sheet} onStartShouldSetResponder={()=>true}>
              ...
              <View style={st.attachRow}>
                ...
                <TouchableOpacity style={st.attachBtn} onPress={()=>setShowLinkField(v=>!v)} activeOpacity={0.8}>
                  <Text style={[st.attachBtnText,showLinkField ? st.attachBtnOn : null]}>{"\uD83D\uDD17"} Link</Text>
                </TouchableOpacity>
              </View>
              {showLinkField && (
              ...
              <View style={st.btnRow}>
                ...
                <TouchableOpacity style={[st.submitBtn,(!postText.trim()||posting) ? st.submitDim : null]}
          ...
          <TouchableOpacity style={st.overlay} activeOpacity={1} onPress={()=>{setShowNew(false);setNewName("");}}>
            <View style={st.sheet} onStartShouldSetResponder={()=>true}>
              ...
              <View style={st.colorRow}>
                {GROUP_COLORS.map(c=>(
                  <TouchableOpacity key={c.key} style={[st.colorSwatch,{backgroundColor:c.value},newColorKey===c.key ? st.colorSwatchOn : null]}
              ...
              <View style={st.btnRow}>
                ...
                <TouchableOpacity style={[st.submitBtn,!newName.trim() ? st.submitDim : null]} onPress={handleCreate} disabled={!newName.trim()}>
        ...
        <KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==="ios"?"padding":"height"}>
          <TouchableOpacity style={st.overlay} activeOpacity={1} onPress={()=>setShowEdit(false)}>
            <View style={st.sheet} onStartShouldSetResponder={()=>true}>
              ...
              <View style={st.colorRow}>
                {GROUP_COLORS.map(c=>(
                  <TouchableOpacity key={c.key} style={[st.colorSwatch,{backgroundColor:c.value},editColorKey===c.key ? st.colorSwatchOn : null]}
              ...
              <View style={st.btnRow}>
                ...
                <TouchableOpacity style={[st.submitBtn,!editName.trim() ? st.submitDim : null]} onPress={handleSaveEdit} disabled={!editName.trim()}>
