...
const GENERAL_SECTIONS = [
  ...
  {
    id: "rituals", title: "Ritual Guidance & Schools of Thought", icon: "📖",
    items: [
      ...
      {open && (
        <View style={ac.body}>
          {section.items.map((item, i) => {
            ...
            return (
              <View key={i} style={[ac.item, i === section.items.length - 1 && { borderBottomWidth: 0 }]}>
  ...
  return (
    <View style={nl.card}>
      ...
      {links.map((link, i) => (
        <TouchableOpacity
          key={link.url}
          style={[nl.row, i < links.length - 1 && nl.rowBorder]}
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.tabRow}>
        {[["general","General"],["health","Health & Medical"]].map(([key, label]) => (
          <TouchableOpacity
            key={key}
            style={[s.tabOpt, tab === key && s.tabOptActive]}
