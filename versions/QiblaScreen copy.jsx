    ...
    <Svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
      <Defs>
        <LinearGradient id="qiblaGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          <Stop offset="100%" stopColor="#1E4A3A" stopOpacity="1" />
        </LinearGradient>
      </Defs>

      {/* Outer ring */}
      <Circle cx={CX} cy={CY} r={R} fill="#FDFAF4" stroke="#D4D0CA" strokeWidth={1.5} />
      ...
      {Array.from({length:72}, (_,i) => {
        ...
        return (
          <Line key={i}
            ...
            stroke={isMajor ? "#C8A96A" : "#D4D0CA"} strokeWidth={isMajor ? 1.5 : 0.8}
          />
        );
      })}
      ...
      {/* Inner circle */}
      <Circle cx={CX} cy={CY} r={R - 50} fill="#EDE6D8" stroke="#D4D0CA" strokeWidth={1} />
      ...
      {/* Ka'bah icon on the ring at bearing position */}
      {(() => {
        ...
        return (
          <G>
            <Circle cx={ix} cy={iy} r={12} fill="#2F5D50" stroke="#FDFAF4" strokeWidth={2} />
            <SvgText x={ix} y={iy + 5} fontSize={12} textAnchor="middle">{"🕋"}</SvgText>
          </G>
        );
      })()}

      {/* Centre dot */}
      <Circle cx={CX} cy={CY} r={6} fill="#2F5D50" />
      <Circle cx={CX} cy={CY} r={2} fill="#C8A96A" />
    </Svg>
  );
}
...
const s = StyleSheet.create({
  ...
  compassWrap:   { alignItems:"center", paddingVertical:12 },

  infoRow:       { flexDirection:"row", gap:10, marginBottom:12 },
  numbersCard:   { backgroundColor:"#FDFAF4", borderRadius:16, borderWidth:1, borderColor:"#D4D0CA", padding:16, justifyContent:"center", gap:12, shadowColor:"#6A4A28", shadowOffset:{width:0,height:2}, shadowOpacity:0.08, shadowRadius:4, elevation:2 },
  numberBlock:   { alignItems:"center" },
  numberLabel:   { fontSize:9, fontWeight:"700", letterSpacing:1.5, color:"#9A8E7A", marginBottom:2 },
  numberValue:   { fontFamily:SERIF, fontSize:26, color:"#2F5D50", lineHeight:30 },
  numberSub:     { fontSize:11, color:"#5A5650", marginTop:1 },
  ...
  instructBody:  { fontSize:12, color:"#5A5650", lineHeight:18 },
