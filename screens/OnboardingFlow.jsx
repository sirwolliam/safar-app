...
// ─────────────────────────────────────────────────────────────────────────────
// Shared step footer — used by every step for consistency.
// Layout: progress dots on top, then [back arrow] [primary action button].
//   total/active  → progress dots
//   onBack        → if provided, shows the back arrow (omit on step 1 / Welcome)
//   onPress       → primary action
//   label         → button text ("Continue", "Begin", etc.)
//   disabled      → dims + disables the primary button
//   dark          → light text/dots for dark backgrounds (Welcome/Ready)
// ─────────────────────────────────────────────────────────────────────────────
function StepFooter({ total = 6, active = 0, onBack, onPress, label = "Continue", disabled = false, dark = false }) {
  return (
    <View style={sf.footer}>
      <View style={sf.dotsRow}>
        <ProgressDots total={total} active={active} dark={dark} />
      </View>
      <View style={sf.navRow}>
        {onBack ? (
          <TouchableOpacity
            style={dark ? [sf.backBtn, sf.backBtnDark] : sf.backBtn}
            onPress={onBack}
            activeOpacity={0.8}
        ...
        <TouchableOpacity
          style={[
            disabled ? sf.btnDisabled : (dark ? sf.btnDark : sf.btn),
            { flex: 1 },
          ]}
          onPress={onPress}
          activeOpacity={0.85}
          disabled={disabled}
        >
          <Text style={disabled ? sf.btnTxtDisabled : (dark ? sf.btnTxtDark : sf.btnTxt)}>{label}</Text>
          <ArrowRight size={18} color={disabled ? T.faint : (dark ? "#0F2218" : "#fff")} weight="bold" />
        </TouchableOpacity>
      </View>
    </View>
  );
...
const sf = StyleSheet.create({
  footer:        { paddingHorizontal: 24, paddingBottom: 36, paddingTop: 14 },
  dotsRow:       { alignItems: "center", marginBottom: 16 },
  navRow:        { flexDirection: "row", alignItems: "center", gap: 12 },
  backBtn:       {
    width: 54, height: 54, borderRadius: 12,
    alignItems: "center", justifyContent: "center",
    borderWidth: 1.5, borderColor: T.border, backgroundColor: T.card,
  },
  backBtnDark:   { borderColor: "rgba(245,240,232,0.25)", backgroundColor: "rgba(255,255,255,0.06)" },
  btn:           {
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10,
    backgroundColor: T.primary, borderRadius: 12, paddingVertical: 17,
  },
  btnDark:       {
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10,
    backgroundColor: T.gold, borderRadius: 12, paddingVertical: 17,
  },
  btnDisabled:   {
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10,
    backgroundColor: T.border, borderRadius: 12, paddingVertical: 17,
  },
  btnTxt:        { fontFamily: SERIF, fontSize: 17, color: "#fff", fontWeight: "600" },
  btnTxtDark:    { fontFamily: SERIF, fontSize: 17, color: "#0F2218", fontWeight: "700" },
  btnTxtDisabled:{ fontFamily: SERIF, fontSize: 17, color: T.faint, fontWeight: "500" },
});
...
const ns = StyleSheet.create({
  ...
  safe:       { flex: 1 },
  body:       { flex: 1, paddingTop: 56, paddingHorizontal: 30 },
  eyebrow:    {
...
function JourneyTypeScreen({ userName, onNext, onBack }) {
        ...
        <View style={jts.cards}>
          {JOURNEY_OPTIONS.map((opt) => {
            ...
            return (
              <TouchableOpacity
                key={opt.id}
                style={isSel ? [jts.card, jts.cardSelected] : jts.card}
                onPress={() => setSelected(opt.id)}
                activeOpacity={0.82}
              >
                <View style={jts.cardTextWrap}>
                  <Text style={isSel ? [jts.cardLabel, jts.cardLabelSelected] : jts.cardLabel}>{opt.label}</Text>
                  <Text style={isSel ? [jts.cardDesc, jts.cardDescSelected] : jts.cardDesc}>{opt.desc}</Text>
                </View>
                {isSel ? (
                  <CheckCircle size={26} color={T.gold} weight="fill" />
                ) : (
                  <View style={jts.radioEmpty} />
                )}
              </TouchableOpacity>
            );
          })}
...
const jts = StyleSheet.create({
  root:         { flex: 1, backgroundColor: T.bg },
  scroll:       { flex: 1 },
  scrollContent:{ paddingHorizontal: 24, paddingTop: 28, paddingBottom: 12 },
  eyebrow:      {
    fontSize: 13, letterSpacing: 2.5, color: T.primary,
    fontWeight: "700", textTransform: "uppercase", marginBottom: 14,
  },
  title:        { fontFamily: SERIF, fontSize: 30, color: T.text, lineHeight: 38, marginBottom: 10 },
  sub:          { fontSize: 16, color: T.sub, lineHeight: 24, marginBottom: 24 },
  cards:        { gap: 12 },
  ...
  cardLabel:    { fontFamily: SERIF, fontSize: 21, color: T.text, fontWeight: "600", marginBottom: 3 },
  cardDesc:     { fontSize: 15, color: T.sub, fontWeight: "400" },
  cardLabelSelected:  { color: "#FFFFFF" },
  cardDescSelected:   { color: "rgba(255,255,255,0.85)" },

  radioEmpty:   { width: 24, height: 24, borderRadius: 12, borderWidth: 1.5, borderColor: T.border },
});
...
function AssistImportScreen({ journeyType, onNext, onSkip, onBack }) {
  ...
  return (
    <SafeAreaView style={as.root}>
      ...
      <View style={as.footer}>
        <View style={as.dotsRowBottom}>
          <ProgressDots total={6} active={3} />
        </View>
        <View style={as.introNavRow}>
          <TouchableOpacity
            style={as.backBtn}
            onPress={onBack}
            activeOpacity={0.8}
            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
          >
            <ArrowRight size={20} color={T.sub} weight="bold" style={{ transform: [{ scaleX: -1 }] }} />
          </TouchableOpacity>
          <TouchableOpacity style={[as.primaryBtn, { flex: 1 }]} onPress={() => setStage("how")} activeOpacity={0.88}>
...
const as = StyleSheet.create({
  ...
  dotsRowBottom:   { alignItems: "center", marginBottom: 8 },
  introNavRow:     { flexDirection: "row", alignItems: "center", gap: 12 },
  backBtn:         {
    width: 54, height: 54, borderRadius: 12,
    alignItems: "center", justifyContent: "center",
    borderWidth: 1.5, borderColor: T.border, backgroundColor: T.card,
...
// ─────────────────────────────────────────────────────────────────────────────
function ReadyScreen({ userName, journeyType, onComplete, onBack }) {
      ...
      <SafeAreaView style={rs.safe}>
        <View style={rs.body}>
          ...
          <View style={rs.iconRing}>
            <StarAndCrescent size={34} color={T.gold} weight="fill" />
          </View>
        ...
        <View style={rs.footer}>
          ...
          <View style={rs.navRow}>
            <TouchableOpacity
              ...
              hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
            >
              <ArrowRight size={20} color="#F5F0E8" weight="bold" style={{ transform: [{ scaleX: -1 }] }} />
            </TouchableOpacity>
...
const rs = StyleSheet.create({
  ...
  footer:  { paddingHorizontal: 24, paddingBottom: 44, paddingTop: 16 },
  dotsRow: { alignItems: "center", marginBottom: 16 },
  navRow:  { flexDirection: "row", alignItems: "center", gap: 12 },
  backBtn: {
    width: 54, height: 54, borderRadius: 12,
    alignItems: "center", justifyContent: "center",
    borderWidth: 1.5, borderColor: "rgba(245,240,232,0.25)", backgroundColor: "rgba(255,255,255,0.06)",
  },
  cta:     {
    flex: 1,
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10,
  ...
  ctaTxt:  { fontFamily: SERIF, fontSize: 17, color: T.darkBg, fontWeight: "700" },
});
    ...
    case 5:
      return (
        <ReadyScreen
          userName={userName}
          journeyType={journeyType}
          onBack={() => setStep(journeyType === "learn" ? 2 : 4)}
