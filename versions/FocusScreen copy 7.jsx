...
export default function FocusScreen({ navigation }) {
  ...
  // ── Focus landing — always show this, navigate to dedicated screens ──
  if (showPicker || !mode) {
    return (
      <SafeAreaView style={s.safe}>
        <View style={s.pickerWrap}>
          <Text style={s.pickerTitle}>Focus Mode</Text>
          <Text style={s.pickerSub}>Choose your practice to begin.</Text>

          {/* Tawaf */}
          <TouchableOpacity style={s.pickerOpt} onPress={() => navigation?.navigate?.("Tawaf")} activeOpacity={0.88}>
            <View style={s.pickerOptLeft}>
              <Text style={s.pickerOptTitle}>{"Taw\u0101f Counter"}</Text>
              <Text style={s.pickerOptHint}>{"Track 7 circuits around the Ka\u02bfbah"}</Text>
            </View>
            <View style={s.pickerBadge}><Text style={s.pickerBadgeText}>7 rounds</Text></View>
          </TouchableOpacity>

          {/* Sa'y */}
          <TouchableOpacity style={s.pickerOpt} onPress={() => navigation?.navigate?.("Saiy")} activeOpacity={0.88}>
            <View style={s.pickerOptLeft}>
              <Text style={s.pickerOptTitle}>{"Sa\u02bfy Tracker"}</Text>
              <Text style={s.pickerOptHint}>{"\u1e62af\u0101 to Marwah and back \u2014 7 lengths"}</Text>
            </View>
            <View style={s.pickerBadge}><Text style={s.pickerBadgeText}>7 lengths</Text></View>
          </TouchableOpacity>

          {/* Dhikr */}
          <TouchableOpacity style={s.pickerOpt} onPress={() => navigation?.navigate?.("Dhikr")} activeOpacity={0.88}>
            <View style={s.pickerOptLeft}>
              <Text style={s.pickerOptTitle}>Dhikr Counter</Text>
              <Text style={s.pickerOptHint}>{"Remember Allah with any dhikr"}</Text>
            </View>
            <View style={s.pickerBadge}><Text style={s.pickerBadgeText}>22 dhikrs</Text></View>
          </TouchableOpacity>
