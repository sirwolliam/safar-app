...
const nc = StyleSheet.create({
  ...
  sub:          { fontSize:13, color:"#7A7060", lineHeight:18, marginBottom:10, flex:1 },
...
export default function MyDuasScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {/* ── Hero header — large title left, Kaaba image fades in from right ── */}
      <View style={s.heroHeader}>
        {/* Kaaba image — right side, fades at left edge via overlay */}
        <Image
          source={require("../assets/kaaba_map.png")}
          style={s.heroImg}
          resizeMode="cover"
        />
        {/* Left solid parchment block covers left portion of image */}
        <View style={s.heroFadeLeft} />
        {/* Gradient fade from parchment to transparent — layered views */}
        <View style={s.heroFade1} />
        <View style={s.heroFade2} />
        {/* Content sits above all overlays */}
        <View style={s.heroContent}>
          <Text style={s.headerTitle}>{"Duʿās"}</Text>
          <Text style={s.headerSub}>{"Your duʿās, organised
...
const s = StyleSheet.create({
  ...
  // Hero header
  heroHeader:  { height:160, position:"relative", overflow:"hidden", backgroundColor:"#EDE6D8" },
  heroImg:     { position:"absolute", right:0, top:0, bottom:0, width:"65%", opacity:0.6 },
  heroFadeLeft:{ position:"absolute", top:0, bottom:0, left:0, width:"45%", backgroundColor:"#EDE6D8" },
  heroFade1:   { position:"absolute", top:0, bottom:0, left:"35%", width:60, backgroundColor:"rgba(237,230,216,0.85)" },
  heroFade2:   { position:"absolute", top:0, bottom:0, left:"47%", width:40, backgroundColor:"rgba(237,230,216,0.4)" },
  heroContent: { position:"absolute", bottom:16, left:20 },
  headerTitle: { fontFamily:SERIF, fontSize:48, fontWeight:"400", color:"#1A1712", lineHeight:54 },
  headerSub:   { fontSize:15, color:"#5A5650", fontWeight:"400", marginTop:3, lineHeight:21 },
