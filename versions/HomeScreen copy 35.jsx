...
import {
  ...
  TextInput, KeyboardAvoidingView, Platform, Animated, FlatList,
} from "react-native";
...
const HERO_SLIDES = [
  ...
  { img:require("../assets/kaaba_map.png"),        tag:"SACRED PLACES",   headline:"Every site.\nEvery duʿā.",        sub:"Learn and explore with location-specific duʿās" },
  { img:require("../assets/checklist_kaaba.jpg"),  tag:"PREPARATION",     headline:"Plan every step\nwith confidence",  sub:"Your personalised checklist, calendar and guide" },
  { img:require("../assets/tab_my_lists.jpg"),     tag:"DUʿĀS",           headline:"Authenticated\nduʿās for every moment", sub:"Verified from major collections. Audio included." },
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.heroWrap}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={s.heroSlide}>
                <ImageBackground source={item.img} style={s.heroSlideBg} imageStyle={s.heroSlideImg}>
                  <View style={s.heroScrim} pointerEvents="none" />
                  <View style={s.heroSlideContent}>
                    <Text style={s.heroSlideTag}>{item.tag}</Text>
                    <Text style={s.heroSlideHeadline}>{item.headline}</Text>
                    <Text style={s.heroSlideSub}>{item.sub}</Text>
                  </View>
                </ImageBackground>
              </View>
            )}
          ...
          {/* Logomark — fixed top-left over carousel */}
          <View style={s.logomark} pointerEvents="none">
            <View style={s.logomarkRing}>
              <Text style={s.logomarkArabic}>{"\u0633"}</Text>
            </View>
            <Text style={s.logomarkText}>SAFAR</Text>
          </View>
          ...
          {/* Slide dots — fixed bottom of hero */}
          <View style={s.heroDots} pointerEvents="none">
            {HERO_SLIDES.map((_, i) => (
              <View key={i} style={i === heroSlide ? [s.heroDot, s.heroDotActive] : s.heroDot} />
            ))}
          </View>

          {/* Greeting — bottom of hero above card */}
          <View style={s.heroGreeting} pointerEvents="none">
            <Text style={s.heroSalam}>{"As-sal\u0101mu \u02bfalaykum"}</Text>
            <Text style={s.heroName}>{userName || "Welcome to Safar"}</Text>
          </View>

          {/* Departure card — sits at bottom, 1/3 overlapping hero */}
        ...
        <View style={s.content}>
          ...
          <View style={s.explainerWrap}>
            <View style={s.explainerLine} />
            <Text style={s.explainerText}>
              {"Your journey. Your plan. Everything in one place."}
            </Text>
            <View style={s.explainerLine} />
          </View>
          ...
          <View style={s.journeyGrid}>
            {[
              { img:require("../assets/kaaba_mixed.png"),     label:"Guide",         sub:"Step-by-step rites",            screen:"Journey", badge:null },
              { img:require("../assets/kaaba_map.png"),       label:"Sacred Places", sub:"Sites & duʿās",       screen:"Map",     badge:null },
              ...
              { img:require("../assets/journey3.png"),        label:"My Calendar",   sub:"Dates & notes",                 screen:"Journey", badge:null },
            ].map((item, i) => (
              <TouchableOpacity key={i} style={s.journeyTile}
                onPress={() => navigation?.navigate?.(item.screen)} activeOpacity={0.88}>
                <ImageBackground source={item.img} style={s.journeyTileBg} imageStyle={s.journeyTileBgImg}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}
            ...
            {[
              { screen:"Focus",       image:require("../assets/07_arafah_gradient.jpg"), label:"Tasbeeh",      sub:"Dhikr counter",   accent:"#1E3D30" },
              { screen:"ImportTrip",  image:require("../assets/tab_dua_library.jpg"),    label:"Quick Add",    sub:"Import booking",  accent:"#2A5242" },
              { screen:"PrayerTimes", image:require("../assets/kaaba_map.png"),          label:"Prayer Times", sub:"Today's salah",  accent:"#1E3D30" },
              { screen:"Qibla",       image:require("../assets/medina.png"),             label:"Qibla",        sub:"Find direction",  accent:"#2A5242" },
              { screen:"WhatToExpect",image:require("../assets/what_to_expect.jpg"),     label:"What to Expect",sub:"Plan your trip", accent:"#1E3D30" },
            ].map((tool, i) => (
              <TouchableOpacity key={i} style={s.toolPill}
                onPress={() => navigation?.navigate?.(tool.screen)} activeOpacity={0.88}>
                <ImageBackground source={tool.image} style={s.toolPillBg} imageStyle={s.toolPillBgImg}>
                  <View style={s.toolPillScrim} />
                  <View style={s.toolPillContent}>
                    <Text style={s.toolPillLabel}>{tool.label}</Text>
                    <Text style={s.toolPillSub}>{tool.sub}</Text>
                  </View>
                </ImageBackground>
              </TouchableOpacity>
            ))}
...
const s = StyleSheet.create({
  ...
  heroSlide:      { width:SW, height:HERO_H },
  heroSlideBg:    { flex:1 },
  heroSlideContent:{ position:"absolute", bottom:180, left:20, right:20 },
  heroSlideTag:   { fontSize:10, fontWeight:"800", letterSpacing:2.5, color:"rgba(200,169,106,0.90)", marginBottom:8 },
  heroSlideHeadline:{ fontFamily:SERIF, fontSize:30, color:"#fff", lineHeight:37, fontWeight:"400" },
  heroSlideSub:   { fontSize:13, color:"rgba(255,255,255,0.72)", fontWeight:"500", marginTop:6, lineHeight:18 },
  heroDots:       { position:"absolute", bottom:82, left:0, right:0, flexDirection:"row", justifyContent:"center", gap:6, alignItems:"center" },
  heroDot:        { width:5, height:5, borderRadius:3, backgroundColor:"rgba(255,255,255,0.35)" },
  heroDotActive:  { width:20, height:5, borderRadius:3, backgroundColor:"#fff" },
  heroImg:  { resizeMode:"cover" },
  heroScrim:{ ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(8,14,10,0.42)" },

  // Logomark
  logomark:     { position:"absolute", top:16, left:20, flexDirection:"row", alignItems:"center", gap:8, zIndex:10 },
  logomarkRing: { width:32, height:32, borderRadius:16, borderWidth:1.5, borderColor:"rgba(255,255,255,0.55)", backgroundColor:"rgba(255,255,255,0.12)", alignItems:"center", justifyContent:"center" },
  logomarkArabic:{ fontFamily:SERIF, fontSize:16, color:"#fff", lineHeight:20 },
  logomarkText: { fontSize:11, fontWeight:"800", letterSpacing:3, color:"rgba(255,255,255,0.75)" },

  // Greeting strip — between hero and card
  greetingStrip:  { paddingHorizontal:20, paddingTop:14, paddingBottom:10 },
  greetingSalam:  { fontSize:13, color:"#5C534A", fontWeight:"500", letterSpacing:0.5, marginBottom:3 },
  greetingName:   { fontFamily:SERIF, fontSize:32, color:"#100E0A", lineHeight:38 },
  ...
  // Greeting
  heroGreeting:      { position:"absolute", bottom:108, left:20, right:20 },
  heroSalam:         { fontSize:13, color:"rgba(255,255,255,0.72)", fontWeight:"500", letterSpacing:0.5, marginBottom:4 },
  heroName:          { fontFamily:SERIF, fontSize:30, color:"#fff", lineHeight:36 },
  heroNamePlaceholder:{ fontFamily:SERIF, fontSize:28, color:"#fff", lineHeight:34, marginBottom:4 },
  heroTagline:       { fontFamily:SERIF, fontSize:19, color:"rgba(255,255,255,0.88)", fontWeight:"400" },
  ...
  departureCard: {
    ...
    overflow:"hidden", borderRadius:20, borderWidth:1, borderColor:"#C8BFB2", backgroundColor:"#FDFAF4",
  },
  ...
  departureDaysNum:  { fontFamily:SERIF, fontSize:52, color:"#1E3D30", lineHeight:56, fontWeight:"600", marginTop:3 },
  ...
  sectionRow:     { flexDirection:"row", alignItems:"center", gap:10, marginBottom:12, marginTop:16 },
  sectionBar:     { width:3, height:18, borderRadius:2, backgroundColor:"#1E3D30", flexShrink:0 },
  sectionLine:    { flex:1, height:1, backgroundColor:"#C8BFB2" },
  sectionHeader:  { marginBottom:12, marginTop:24 },
  explainerWrap:   { marginTop:6, marginBottom:4, paddingVertical:8 },
  explainerLine:   { height:1, backgroundColor:"#C8BFB2", marginBottom:8 },
  explainerText:   { fontSize:14, color:"#5C534A", fontWeight:"500", lineHeight:21, textAlign:"center" },
  ...
  duaBtnPrimaryTxt:{ fontFamily:SERIF, fontSize:15, color:"#fff", fontWeight:"600", letterSpacing:0.3 },
  ...
  journeyTileBg:   { flex:1, justifyContent:"space-between" },
  journeyTileBgImg:{ resizeMode:"cover" },
  ...
  toolPillBg:     { flex:1 },
  toolPillBgImg:  { resizeMode:"cover" },
  toolPillScrim:  { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(6,12,8,0.55)" },
  toolPillContent:{ position:"absolute", bottom:0, left:0, right:0, padding:14 },
  toolPillLabel:  { fontFamily:SERIF, fontSize:15, color:"#fff", fontWeight:"600", marginBottom:2 },
  toolPillSub:    { fontSize:11, color:"rgba(255,255,255,0.75)", fontWeight:"500" },
});
