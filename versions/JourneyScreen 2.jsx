...
import {
  ...
  UIManager, Modal, TextInput, KeyboardAvoidingView, Linking,
  Alert,
} from "react-native";
...
const CARD_SUGGESTIONS = {
  ...
  checklist: ["Pack ihram clothing", "Renew passport", "Book accommodation near Haram", "Get travel insurance", "Exchange currency to SAR"],
...
function AddCardModal({ visible, onAdd, onClose }) {
  ...
  const reset = () => {
    setStep("type"); setCardType(null);
    setText(""); setTitle(""); setUrl("");
    setSelectedDua(null); setDuaSearch("");
  };
            ...
            <View style={am.sheetHeader}>
              {step === "form" ? (
                <TouchableOpacity onPress={() => setStep("type")}>
                  <Text style={am.back}>{"\u2039"} Back</Text>
                </TouchableOpacity>
              ) : (
                <View style={{ width:50 }} />
              )}
              <Text style={am.sheetTitle}>
                {step === "type" ? "Add to your board" : CARD_TYPES.find(t=>t.key===cardType)?.label}
              </Text>
              <TouchableOpacity onPress={handleClose}>
                <Text style={am.closeBtn}>{"\u2715"}</Text>
              </TouchableOpacity>
            </View>
              ...
              <View style={am.typeGrid}>
                {CARD_TYPES.map(type => (
                  <TouchableOpacity key={type.key} style={am.typeOpt}
                    onPress={() => { setCardType(type.key); setStep("form"); }}
                    activeOpacity={0.85}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                ...
                {cardType === "note" && (
                  <>
                    ...
                    <TextInput style={[am.input, am.inputMulti]}
                      placeholder="e.g. My intention for this journey…"
                ...
                {cardType === "checklist" && (
                  <>
                    ...
                    <TextInput style={am.input}
                      placeholder="e.g. Pack ihram clothing"
                      placeholderTextColor={colors.subtext}
                ...
                {cardType === "dua" && (
                  <>
                    ...
                    <View style={am.searchBar}>
                      ...
                      <TextInput style={am.searchInput}
                        placeholder="Search by title or stage…"
                        placeholderTextColor={colors.subtext}
                        value={duaSearch} onChangeText={setDuaSearch}
                        autoFocus />
                    </View>
                    {filteredDuas.map(dua => (
                      <TouchableOpacity key={dua.id}
                        style={[am.duaRow, selectedDua?.id===dua.id && am.duaRowSelected]}
                ...
                {cardType === "link" && (
                  <>
                    ...
                    <TextInput style={am.input}
                      ...
                      keyboardType="url" autoCapitalize="none" autoCorrect={false} autoFocus />
                    <Text style={am.fieldLabel}>Title (optional)</Text>
                    <TextInput style={am.input}
                      placeholder="e.g. My hotel booking"
                      placeholderTextColor={colors.subtext}
...
function BoardCard({ card, onToggle, onDelete }) {
  ...
  if (card.type === "checklist") {
    return (
      <TouchableOpacity style={[bc.card, bc.checklistCard]}
        onPress={() => onToggle(card.id)} onLongPress={handleLongPress} activeOpacity={0.85}>
    ...
    return (
      <TouchableOpacity style={[bc.card, bc.duaCard]} onLongPress={handleLongPress} activeOpacity={0.9}>
        <View style={bc.duaHeader}>
          ...
          <Text style={bc.duaPinEmoji}>{"\uD83E\uDD32"}</Text>
        </View>
  ...
  if (card.type === "link") {
    return (
      <TouchableOpacity style={[bc.card, bc.linkCard]}
        onPress={() => Linking.openURL(card.url)} onLongPress={handleLongPress} activeOpacity={0.85}>
        <View style={bc.linkIconWrap}>
...
export default function JourneyScreen({ navigation }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(BOARD_KEY)
      .then(v => { if (v) setBoardCards(JSON.parse(v)); })
      .catch(() => {});
  }, []);

  const saveBoard = (cards) => {
    setBoardCards(cards);
    AsyncStorage.setItem(BOARD_KEY, JSON.stringify(cards)).catch(() => {});
  };

  const addCard = (card) => {
    const next = [card, ...boardCards];
    saveBoard(next);
  ...
  const toggleCard = (id) => {
    saveBoard(boardCards.map(c => c.id === id ? { ...c, done: !c.done } : c));
  };

  const deleteCard = (id) => {
    saveBoard(boardCards.filter(c => c.id !== id));
  };
  ...
  return (
    <SafeAreaView style={jn.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }}>
        ...
        <View style={jn.modeWrap}>
          <View style={jn.modeRow}>
            {[
              { key:"umrah", emoji:"\uD83D\uDD4B", name:"Umrah", sub:"Lesser pilgrimage" },
              { key:"hajj",  emoji:"\u26F0\uFE0F",  name:"Hajj",  sub:"Annual pilgrimage" },
            ].map(m => (
              <TouchableOpacity key={m.key}
                style={[jn.modeOpt, mode===m.key && jn.modeOptActive]}
        ...
        {/* ── 3-tab bar: All Steps · Completed · My Board ── */}
        <View style={jn.tabs}>
          {[["all","All Steps"],["done","Completed"],["board","My Board"]].map(([key,label]) => (
            <TouchableOpacity key={key}
              style={[jn.tab, activeTab===key && jn.tabActive]}
              onPress={() => setActiveTab(key)} activeOpacity={0.8}>
              <Text style={[jn.tabLabel, activeTab===key && jn.tabLabelActive]}>{label}</Text>
              {key === "board" && boardCards.length > 0 && (
                <View style={jn.tabBadge}>
                  <Text style={jn.tabBadgeText}>{boardCards.length}</Text>
                </View>
              )}
            </TouchableOpacity>
          ))}
          ...
          <View style={jn.boardWrap}>
            ...
            <View style={jn.boardHeader}>
              <View>
                ...
                {boardChecklist > 0 && (
                  <Text style={jn.boardProgress}>{boardDone} of {boardChecklist} tasks done</Text>
                )}
              </View>
              <TouchableOpacity style={jn.addCardBtn}
                onPress={() => setShowAddCard(true)} activeOpacity={0.85}>
            ...
            {boardCards.length === 0 && (
              <View style={jn.boardEmpty}>
                ...
                <Text style={jn.boardEmptyBody}>
                  Add notes, checklist items, pinned du\u02bf\u0101\u02beS, and useful links to personalise your journey.
                </Text>
                <TouchableOpacity style={jn.boardEmptyBtn}
                  onPress={() => setShowAddCard(true)} activeOpacity={0.85}>
            ...
            {/* Cards */}
            {boardCards.map(card => (
              <BoardCard key={card.id} card={card} onToggle={toggleCard} onDelete={deleteCard} />
            ))}
      ...
      {/* Floating + button on board tab */}
      {activeTab === "board" && boardCards.length > 0 && (
        <TouchableOpacity style={jn.fab} onPress={() => setShowAddCard(true)} activeOpacity={0.88}>
...
const jn = StyleSheet.create({
  ...
  boardHint:         { fontSize:typography.tiny, color:colors.border, textAlign:"center", marginTop:spacing(1.5) },
