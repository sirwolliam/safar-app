  ...
  return (
    <SafeAreaView style={jn.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }} contentContainerStyle={jn.scroll}>
        ...
        {/* ── 4. Collapsible Checklist ── */}
        <View style={jn.collapseCard}>
          <TouchableOpacity style={jn.collapseHeader}
            onPress={() => {
              LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
              setChecklistOpen(v => !v);
            }}
            activeOpacity={0.85}>
            <View style={jn.collapseHeaderLeft}>
              <Text style={jn.collapseIcon}>{"✓"}</Text>
              <View>
                <Text style={jn.collapseTitle}>{mode === "umrah" ? "Umrah" : "Hajj"} Checklist</Text>
                <Text style={jn.collapseSub}>{completedCount} of {steps.length} steps done</Text>
              </View>
            </View>
            <Text style={jn.collapseChev}>{checklistOpen ? "▲" : "▼"}</Text>
          </TouchableOpacity>
          {checklistOpen && (
            <View style={jn.collapseBody}>
              {steps.map((step, i) => (
                <View key={step.id} style={i < steps.length - 1 ? [jn.checkRow, jn.checkRowBorder] : jn.checkRow}>
                  <View style={step.done ? [jn.checkBox, jn.checkBoxDone] : jn.checkBox}>
                    {step.done ? <Text style={jn.checkMark}>{"✓"}</Text> : null}
                  </View>
                  <View style={{ flex:1 }}>
                    <Text style={step.done ? [jn.checkName, jn.checkNameDone] : jn.checkName}>{step.name}</Text>
                    <Text style={jn.checkSub}>{step.sub}</Text>
                  </View>
                </View>
              ))}
              <TouchableOpacity style={jn.collapseViewAll}
                onPress={() => navigation?.navigate?.(mode === "umrah" ? "UmrahGuide" : "HajjGuide")}>
                <Text style={jn.collapseViewAllTxt}>View full guide {"›"}</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
        ...
        <View style={jn.collapseCard}>
          <TouchableOpacity style={jn.collapseHeader}
            ...
            <View style={jn.collapseHeaderLeft}>
              <Text style={jn.collapseIcon}>{"🤲"}</Text>
              <View>
                <Text style={jn.collapseTitle}>{mode === "umrah" ? "Umrah" : "Hajj"} Du'āʾs</Text>
                <Text style={jn.collapseSub}>Key du'āʾs for your journey</Text>
              </View>
            </View>
            <Text style={jn.collapseChev}>{duasOpen ? "▲" : "▼"}</Text>
          </TouchableOpacity>
          {duasOpen && (
            <View style={jn.collapseBody}>
              {steps.filter(s => s.duas?.length > 0).map((step, si) => (
                <View key={step.id}>
                  <Text style={jn.duaStageLabel}>{step.name}</Text>
                  {step.duas.map((dua, di) => (
                    <TouchableOpacity key={dua.id} style={jn.duaRow}
                      onPress={() => { setPlayerStep(step); setDuaIndex(di); setPlayerDua(dua); }}
                      activeOpacity={0.85}>
                      <View style={{ flex:1 }}>
                        <Text style={jn.duaTitle}>{dua.title}</Text>
                        <Text style={jn.duaArabicPreview} numberOfLines={1}>{dua.arabic}</Text>
                      </View>
                      <Text style={jn.duaArrow}>{"›"}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              ))}
              {steps.every(s => !s.duas?.length) && (
                <Text style={jn.duaEmpty}>Du'āʾs will appear here as you progress through the guide.</Text>
              )}
            </View>
          )}
        </View>

        {/* ── 6. Sacred Places map card ── */}
        <TouchableOpacity style={jn.sacredCard}
          onPress={() => navigation?.navigate?.("Map")}
          activeOpacity={0.88}>
          <ImageBackground
            source={require("../assets/kaaba_map.png")}
            style={jn.sacredCardBg}
            imageStyle={{ resizeMode:"cover" }}>
            <View style={jn.sacredScrim} />
            <View style={jn.sacredContent}>
              <Text style={jn.sacredEyebrow}>SACRED PLACES</Text>
              <Text style={jn.sacredTitle}>Makkah & Madinah Map</Text>
              <Text style={jn.sacredSub}>Du'āʾs for each sacred location</Text>
            </View>
            <View style={jn.sacredArrowWrap}>
              <Text style={jn.sacredArrow}>{"›"}</Text>
            </View>
          </ImageBackground>
        </TouchableOpacity>

        {/* ── 7. What to Expect + half row ── */}
        <View style={jn.halfRow}>
          ...
          <TouchableOpacity style={jn.halfCard}
            ...
            activeOpacity={0.88}>
            <View style={[jn.halfCard, { backgroundColor:colors.card, borderWidth:1, borderColor:colors.border, justifyContent:"flex-end", padding:spacing(1.5) }]}>
              <Text style={jn.halfCardCategory}>{"COMMUNITY"}</Text>
              <Text style={[jn.halfCardTitleWhite, { color:colors.text }]}>My Groups</Text>
              <Text style={[jn.halfCardSubWhite, { color:colors.subtext }]}>{"Share milestones\nand updates"}</Text>
            </View>
          </TouchableOpacity>
...
const jn = StyleSheet.create({
  ...
  // ── Collapsible cards (checklist + duas) ─────────────────────────────────────
  collapseCard:       { backgroundColor:colors.card, borderRadius:radius.lg, borderWidth:1, borderColor:colors.border, marginBottom:spacing(1.5), overflow:"hidden", ...shadows.card },
  collapseHeader:     { flexDirection:"row", alignItems:"center", justifyContent:"space-between", padding:spacing(2) },
  collapseHeaderLeft: { flexDirection:"row", alignItems:"center", gap:spacing(1.25), flex:1 },
  collapseIcon:       { fontSize:20, width:32, textAlign:"center" },
  collapseTitle:      { fontFamily:SERIF, fontSize:17, color:colors.text, marginBottom:2 },
  collapseSub:        { fontSize:12, color:colors.subtext },
  collapseChev:       { fontSize:11, color:colors.subtext, fontWeight:"600" },
  collapseBody:       { borderTopWidth:1, borderTopColor:colors.border },

  // Checklist rows inside collapse
  checkRow:       { flexDirection:"row", alignItems:"center", gap:spacing(1.5), paddingHorizontal:spacing(2), paddingVertical:spacing(1.5) },
  checkRowBorder: { borderBottomWidth:1, borderBottomColor:colors.border },
  checkBox:       { width:22, height:22, borderRadius:11, borderWidth:1.5, borderColor:colors.border, alignItems:"center", justifyContent:"center", flexShrink:0 },
  checkBoxDone:   { backgroundColor:colors.primary, borderColor:colors.primary },
  checkMark:      { fontSize:11, color:"#fff", fontWeight:"700" },
  checkName:      { fontFamily:SERIF, fontSize:15, color:colors.text, marginBottom:1 },
  checkNameDone:  { color:colors.subtext, textDecorationLine:"line-through" },
  checkSub:       { fontSize:12, color:colors.subtext },
  collapseViewAll:    { margin:spacing(2), alignItems:"center" },
  collapseViewAllTxt: { fontSize:14, color:colors.primary, fontWeight:"500" },

  // Duas rows inside collapse
  duaStageLabel:  { fontSize:10, fontWeight:"700", letterSpacing:1.5, color:colors.accent, paddingHorizontal:spacing(2), paddingTop:spacing(1.5), paddingBottom:spacing(0.75) },
  duaRow:         { flexDirection:"row", alignItems:"center", gap:spacing(1.25), paddingHorizontal:spacing(2), paddingVertical:spacing(1.25), borderTopWidth:1, borderTopColor:colors.border },
  duaTitle:       { fontFamily:SERIF, fontSize:15, color:colors.text, marginBottom:3 },
  duaArabicPreview:{ fontSize:13, color:colors.subtext, textAlign:"right" },
  duaArrow:       { fontSize:20, color:colors.border },
  duaEmpty:       { fontSize:14, color:colors.subtext, textAlign:"center", padding:spacing(2.5), fontStyle:"italic" },

  // ── Sacred places full-width card ────────────────────────────────────────────
  sacredCard:     { borderRadius:radius.xl, overflow:"hidden", marginBottom:spacing(1.5), height:120, ...shadows.card },
  sacredCardBg:   { flex:1, flexDirection:"row", alignItems:"center" },
  sacredScrim:    { ...StyleSheet.absoluteFillObject, backgroundColor:"rgba(10,8,4,0.38)" },
  sacredContent:  { flex:1, padding:spacing(2) },
  sacredEyebrow:  { fontSize:9, fontWeight:"700", letterSpacing:1.8, color:"rgba(240,228,200,0.8)", marginBottom:4 },
  sacredTitle:    { fontFamily:SERIF, fontSize:20, color:"#fff", fontWeight:"400", marginBottom:3 },
  sacredSub:      { fontSize:13, color:"rgba(255,255,255,0.75)" },
  sacredArrowWrap:{ paddingRight:spacing(2) },
  sacredArrow:    { fontSize:28, color:"rgba(255,255,255,0.7)" },
});
