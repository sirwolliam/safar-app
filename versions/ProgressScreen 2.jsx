  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.heroWrap}>
        {journey === "Umrah"
          ? <UmrahHero w={SW} h={HERO_H} />
          : <HajjHero  w={SW} h={HERO_H} />
        }
      </View>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        {steps.map((step, i) => {
          ...
          return (
            <View key={step.id} style={[s.stepCard, done && s.stepCardDone]}>
              <TouchableOpacity
                style={s.stepRow}
                onPress={() => setExpanded(open ? null : step.id)}
                activeOpacity={0.85}
              >
                <TouchableOpacity style={[s.check, done && s.checkDone]} onPress={() => toggleStep(step.id)}>
...
const createStyles = (colors) => StyleSheet.create({
  ...
  chevron:   { fontSize: 9, color: colors.border },
