...
*Last consolidated: 2026-07-22 — full update from 2026-07-18 through 2026-07-22 sessions (Opus 4.7). Prior merge: 2026-07-11 from SAFAR_TDD.md (Jun 20), SAFAR_TDD_MASTER.md (Jun, Sessions 1–7), and SAFAR_HANDOFF.md (Jul 11).*
...
> These colors were updated 2026-07-18 to match the live HomeScreen pillar card backgrounds and the `PillarList.jsx` shared component. The previous darker values (`#2E4560`, `#2D4F32`, `#4E3414`, `#3D2240`) were from `PillarIntroScreen.jsx`'s legacy palette — migrated as part of the shared `PillarList` extraction.
...
- No emoji anywhere in the UI — Phosphor icons only. **One documented exception:** country flag imagery on CurrencyScreen uses SVG assets from `flagAssets.js` (MIT-licensed from `lipis/flag-icons`), not emoji or Phosphor substitutes — flags are content, not chrome.
...
9. **File location conventions:**
   ...
   - **Shared components at project root** (alongside `App.js`): `HeaderPatternBg.jsx`, `SafarAssistCard.jsx`, `PillarList.jsx`, `KaabahIcon.jsx`, `flagAssets.js` — imported from screens as `"../ComponentName"`
...
- **PrepareStack:** PrepareMain (ProfileScreen), Notes, CurrencyConverter, Support, Settings — (**BookmarksScreen deleted 2026-07-22**, replaced by MyBoardScreen in JourneyStack)
- **Root Stack (full-screen, no tab bar):** Onboarding, MainTabs, DuaDetail, StepGuide (ProgressScreen), PracticeLearn, PilgrimageDuas, SafarAssist, SacredPlaces — (PrintOffline still registered but unreachable after 2026-07-18 ProfileScreen removal of "Save for Offline" row)
...
- `BoardScreen.jsx` — **deleted 2026-07-22**, replaced by MyBoardScreen
- `HubScreen.jsx` — **deleted 2026-07-22**, superseded by HubContainerScreen
- `PlanHubScreen.jsx` — **deleted 2026-07-22**, superseded by HubContainerScreen
- `BookmarksScreen.jsx` — **deleted 2026-07-22**, functionality merged into MyBoardScreen (unified board)
...
- **Header tier system — updated 2026-07-18.** The original three-tier system (2026-07-12) has been consolidated. Warm Slate Tier 2 (`#3A3545`) is retired — Settings and Support were migrated to Sage Ornate Header on 2026-07-18. The current system:
  - **Tier 1 — Pillar Hub Header:** tall image-backed header (height 260) with LinearGradient scrim, gold-bordered circle icon, serif fontSize 38 white title, light subtitle. Used on: ToolsScreen, HubContainerScreen (Plan/Learn/Practice/Connect), SafarAssistScreen.
  - **Sage Ornate Header — all utility/content screens:** sage green `#4A5C48` background with gold `HeaderPatternBg` bleeding to true top, centered serif fontSize 38 title in `#FDFAF4`, light chip buttons on top row (`#FDFAF4` bg, no border, dark icons). `StatusBar barStyle="light-content"`. Used on: Currency, MyBoard, Notes, PrayerTimes, Qibla, Settings, Support.
  - **Sage Solid Header — CalendarScreen only:** sage green `#4A5C48` background, no gold pattern, centered serif title. Documented exception — the solid dark header frames the calendar grid visually and echoes the Personal category dot color. Do not extend to other screens without design review.
  - **Implementation notes:** chip buttons on sage headers use `#FDFAF4` bg with NO border (the `#DDD5C0` border from light-on-parchment headers is invisible on sage — deliberately removed). Add buttons (`Plus` icon) on Bookmarks/Notes use `#FDFAF4` fill with `#1A1410` icon (reversed from the previous `#4A5C48` fill with white icon, which disappeared against the sage header). PrayerTimes notify active state uses gold `#C8A96A` (the correct brand accent for active toggles on dark surfaces).
  - **Sub-header text on Sage Ornate screens** uses `#FDFAF4` (light, matching title) at fontSize 14–15, consistent across Bookmarks, Notes, PrayerTimes, Qibla. Established 2026-07-18.
...
- **Gold text on light backgrounds is a low-contrast pattern** — when gold text appears on parchment or `#FDFAF4`, use the darker `#B08F52` instead. `#C8A96A` is fine on dark surfaces. Established 2026-07-18 after ProfileScreen eyebrow labels were near-illegible on cream.
- **Minimum text size: 13px.** No tiny text anywhere in the app. Eyebrow labels, category tags, hints — all 13px minimum. Established 2026-07-18.
- **Copy standard: the word "rites" is not used in the app.** Describe pilgrimage acts in plain English ("step," "stage," "part") or with the specific Arabic name (Ṭawāf, Sa'y, Wuqūf, etc.). Removed app-wide 2026-07-18 across 14 files, 19 occurrences. If new copy includes "rites," flag it and rewrite. Scholar review pending for all Islamic content before submission.
...
- **SettingsScreen AboutSafarModal** — inline modal is stale vs HomeScreen's updated AboutModal (which has PillarList, OrnamentDivider, condensed intro). Extract to shared component.
- **SettingsScreen palette debt** — body still uses superseded colors (`#E8DDD0` bg, `#100E0A` text, `#C8BFB2` borders, `#1E3D30` switch track). Needs migration.
- **PrayerTimesScreen + QiblaScreen palette debt** — body content still uses superseded `#E8DDD0`/`#100E0A`/`#F5EDE0`/`#C8BFB2`/`#1E3D30` palette below the header. Header migrated, body not yet.
- **CalendarScreen Hijri date** — placeholder text, needs a real conversion library.
- **SafarAssistScreen + PilgrimageDuasScreen** — still in Root Stack (no tab bar). Should they move to a tab stack per the bottom-tab-visibility rule?
- **Dev build requirements** — not yet started. Needs: EAS setup, bundle identifier, Apple Developer account, permission strings, app icon/splash, Firebase config audit. Deferred until design work is complete.
- **Security blockers before App Store:** Claude API key in client bundle needs backend proxy. Currency Exchange API key treat as compromised, rotate.
...
### Completed / solid
- **HomeScreen** — updated 2026-07-22. Greeting + days-to-go counter moved to parchment section above hero (serif name, compact "42 days to go" line right-aligned). Hero carousel (3 slides, 12-second interval) with updated copy: Welcome ("Your companion for Hajj and Umrah"), Media ("Educate. Prepare. Be ready."), Duas ("Duas for Every Moment"). Diacritics removed from hero copy — "Duas" not "Du'ās". Hero eyebrow labels bumped from 10pt to 12pt. 2×2 pillar tiles with stat badges and ornament divider. AboutModal ("What is Safar?") updated with PillarList shared component, OrnamentDivider, condensed intro line. My Journey card links fixed — Board and Contacts use cross-tab nav with returnToTab, Checklist points to ChecklistsScreen, arrow goes to Journey tab. Prayer times, journey continuation card unchanged.
- **BookmarksScreen** — **DELETED 2026-07-22.** Replaced by unified MyBoardScreen. All bookmark functionality merged into the board.
- **MyBoardScreen** — complete redesign 2026-07-22. Unified personal board replacing both the old MyBoardScreen and BookmarksScreen. Six-category filter system (Duas gold `#C8A96A`, Notes olive `#66572E`, Checklists sage `#4A5C48`, Videos coral `#B8524A`, Podcasts purple `#6B5080`, Links navy `#445870`). Each card shows colored dot + category label (no "SAVED" text). Star icon for favorites (replaces old pin concept — `starred || pinned` for backward compat). Favorites horizontal strip at top (replaces old "Pinned" strip). Category filter pill bar below header (scrollable, counts per category). Tap-to-open routing: bookmarked duas → DuaDetailScreen, bookmarked media/links → browser, user-created content → edit modal. Long-press to delete any card with confirmation. Delete button inside edit modal. "Clear favorites" link next to Favorites header. "Clear entire board" link at bottom of scroll (destructive confirmation). Two-column masonry grid. Add modal with compact pill type selector (Note/Checklist/Dua/Link). Quranic verse as closing inspiration on empty state. Sage Ornate Header with "＋ Add" pill button.
- **NotesScreen** — updated 2026-07-22. Large cards, tag system (Before/During/After/Custom), divider line, search, grid/list toggle. Add button changed to pill format ("＋ Add note"). Subhead updated to "Record or journal your intentions, reflections, and reminders." Voice-to-text option added via expo-speech-recognition (graceful fallback if not installed). Save button moved to sticky bottom footer (like SafarAssist paste mode). Trash icon visible on each card (grid and list views) for delete with confirmation. Sage Ornate Header with sub-text in `#FDFAF4`.
- **PracticeLearnScreen** — queue manager, browse section, quick start sets, large hero "Start Practice" button
- **MediaScreen** — full dark theme (`#000000`), carousel hero, learning paths, browse by topic, horizontal video/podcast/article rows, all text sans-serif except page title. Updated 2026-07-22: back button moved into header (top-left, static light chip), title + icon centered, "Videos, podcasts, and articles" tagline added between title and description. Bookmark action updated to save to unified board with `sourceCategory` (video/podcast/article), `sourceUrl`, and `sourceTitle` — enables proper filtering on MyBoardScreen. Toast says "Saved to My Board" and navigates to MyBoard.
- **WhatToExpectScreen** — hub-style redesign with photo header, gradient overlay, accordion sections, Phosphor icons
- **ProfileScreen (Prepare)** — hub header (shortened from 260px to 170px, 2026-07-22), section pills, profile card, 2×2 personal tiles (height reduced from 110 to 90, padding 12, all tiles same color `rgba(58,53,69,0.82)` — no gradient shimmer). First tile changed from Bookmarks → "My Board" with PushPin icon (2026-07-22). Shop banner + affiliate tiles unified into single card with nested shadow/clip pattern (2026-07-18). "Save for Offline" row removed from resources (2026-07-18, PrintOfflineScreen retired). Gold eyebrow labels bumped to 13pt, darkened to `#B08F52` on light backgrounds.
- **DuaDetailScreen** — complete: large centered Arabic, geometric pattern header, prev/next navigation with counter, transliteration/translation toggles, speed control, bookmark icon wired to unified board via bookmarkStore (2026-07-22 — toast says "Saved to My Board", navigates to MyBoard). Only gap is mock (non-real) audio — see note below, deprioritized.
- **SafarAssistScreen** — **complete redesign 2026-07-22.** Hub Header with `safar-assist-card3.png`, light parchment theme throughout (replaced the old dark `#0F2419` theme entirely). Five-step flow: Step 0 (landing page with import-source logos from `assets/safar_import_logos/`, 2×2 feature grid, condensed intro text, sage green CTA, trust lines with Timer + ShieldCheck icons), Step 1 (four input options as Tools Row cards — voice/upload/photo/paste — with sticky "Read my booking" button in paste mode), Step 2 (processing with sage green spinner), Step 3 (review with editable field rows, grouped by section, sage green "Save to Safar" button), Step 4 (done with Quranic verse). Hub Header shows on Step 0 only; Steps 1–4 use a compact header ("Safar Assist" title, back arrow). Gold step-progress bar with ghosted step names. All text minimum 13px. Section titles centered on Step 0. No dark green surfaces anywhere.
- **PillarIntroScreen** — refactored 2026-07-18 to use shared `PillarList.jsx` component. Migrated off legacy darker pillar colors to current palette. Greeting + "Start exploring" button preserved. Reached as the final step of OnboardingFlow (`navigation.replace("PillarIntro", { userName })`).
- **SettingsScreen / SupportScreen** — migrated to Sage Ornate Header 2026-07-18 (retired the Warm Slate `#3A3545` Tier 2 treatment from 2026-07-12). Both now use sage green `#4A5C48` with gold `HeaderPatternBg`, centered serif fontSize 38 title, light chip back buttons. SupportScreen previously de-emojified (Phosphor icons + custom KaabahIcon throughout). ⚠️ **Known duplication:** SettingsScreen has an inline `AboutSafarModal` that's stale vs HomeScreen's updated `AboutModal` (which has pillars, OrnamentDivider, condensed intro). Extract to shared component in a follow-up session. ⚠️ **Known palette debt:** SettingsScreen body still uses superseded colors (`#E8DDD0` bg, `#100E0A` text, `#C8BFB2` borders, `#1E3D30` switch track). Needs a full palette migration pass — separate from the header work done 2026-07-18.
...
- **Ornate Header pattern** — applied across all utility/content screens as of 2026-07-22 (Checklists, ChecklistDetail, MyBoard, MyContacts, Notes, PrayerTimes, Qibla, PracticeLearn, Calendar, Currency, Settings, Support). Full spec in the header tier system (Section 7). BookmarksScreen deleted.
- **SafarAssistCard** — reusable shared component built 2026-07-13, wired into 6 screens with contextual per-screen copy (Checklists, Calendar, Notes, MyContacts, HubContainerScreen Plan hub, HubContainerScreen Connect hub) plus migrated from JourneyScreen's old one-off card. Added to Plan and Connect hubs 2026-07-22 with per-pillar subtitle copy, wrapped in `marginHorizontal: 16` to match list card width.
- **bookmarkStore.js — unified storage layer (rewritten 2026-07-22).** All saved content (bookmarks, notes, checklists, duas, links) lives in one AsyncStorage key: `safar_journey_board_v1`. Bookmarks from the app (saved duas, media) are stored as board cards with `type: "bookmark"` and cached source content (`sourceTitle`, `sourceArabic`, `sourceTranslation`, `sourceUrl`, `sourceCategory`). Backward-compatible function signatures: `isDuaBookmarked`, `toggleDuaBookmark`, `getDuaBookmarks`, `isMediaBookmarked`, `toggleMediaBookmark`, `getMediaBookmarks`. New board CRUD exports: `getBoardCards`, `addBoardCard`, `updateBoardCard`, `deleteBoardCard`, `toggleBoardPin`, `invalidateBoardCache`. One-time migration (`runBookmarkMigration()`) converts legacy `safar_bookmarks_v1` and `safar_bookmark_media_v1` data to board cards, resolving dua IDs via `getDuaById()`. Migration triggered from App.js useEffect on launch. In-memory cache with `invalidateBoardCache()` export.
- **HubContainerScreen Plan hub hero card** — refreshed 2026-07-22: sage green gradient (`#3A4F3A` → `#2A3E2A`), Sparkle icon on CTA, aligned margins to 16px, warm shadow, serif title, eyebrow bumped to 13pt. Three states: empty (import CTA), no-date (review CTA), has-date (day countdown + review CTA).
...
- **Confirmed navigator map (partial, from direct App.js reads this session) — refer to this before assuming a screen's stack membership:**
  - `HomeStack` (tab `Home`): HomeMain, Hub, PlanHub, LearnHub, PracticeHub, ConnectHub, HubContainer, UmrahGuide, HajjGuide, WhatToExpect, Groups, Guides, Shop, Media, Notes, Settings, Notifications, **Calendar**, **Checklists**, **ChecklistDetail**
...
- **`PillarList.jsx`** — shared component (2026-07-18) rendering the four pillars (Learn/Practice/Plan/Connect) with pillar-identity colors, gold Phosphor icons, and one-line descriptions. Used by `PillarIntroScreen` (onboarding final step) and `HomeScreen`'s `AboutModal` ("What is Safar?" overlay). Props: `showFooterNote` (bool, default true — renders "Duas live at the center of every tab — always one tap away"). Single source of truth for pillar content — update here, both surfaces reflect the change. Pillar identity colors match the HomeScreen pillar card backgrounds: Learn `#446655`, Practice `#66572E`, Plan `#445870`, Connect `#584260`.
- **`flagAssets.js`** — shared module (2026-07-18) containing 14 SVG country flag strings (SAR, GBP, USD, EUR, PKR, IDR, INR, BDT, MYR, EGP, TRY, NGN, CAD, AUD), keyed by currency code. Source: `lipis/flag-icons` (MIT license). Rendered via `SvgXml` from `react-native-svg`. Used by `CurrencyScreen`. Flag imagery is a documented exception to the no-emoji rule — resolved via SVG assets, not emoji or Phosphor substitutes.
...
- **Applied 2026-07-18:** `CurrencyScreen.jsx` (full palette migration + Ornate Header + Tools Row grammar + SVG flags via `flagAssets.js`), `SettingsScreen.jsx`, `SupportScreen.jsx` — all three migrated to Sage Ornate Header. CalendarScreen confirmed visually with sage green `#4A5C48` Sage Solid Header (documented exception — no gold pattern, solid color frames the calendar grid).
- **Sage Ornate Header screens (complete list as of 2026-07-22):** ChecklistsScreen, ChecklistDetailScreen, MyBoardScreen, MyContactsScreen, NotesScreen, PrayerTimesScreen, QiblaScreen, PracticeLearnScreen, CurrencyScreen, SettingsScreen, SupportScreen. CalendarScreen uses Sage Solid variant. BookmarksScreen deleted.
- ⚠️ **StatusBar barStyle:** all Sage Ornate/Solid screens should have `barStyle="light-content"`. Confirmed set on: CurrencyScreen, SettingsScreen, SupportScreen, CalendarScreen, MyBoardScreen, SafarAssistScreen. Other screens may need an audit — small task.
...
- Share Extension — for saving external links/media to Board (requires dev build)
...
- **Unified Bookmarks/Board "My Saved" surface (v2)** — current My Board serves as the unified surface, but the interaction model (star vs pin terminology, board vs bookmark concept) could be further simplified in a future pass

### Recently built (moved from planned → completed this session block)
- ~~Smart checklists~~ → ChecklistsScreen/ChecklistDetailScreen (built 2026-07-13)
- ~~SafarAssist~~ → SafarAssistScreen complete redesign (2026-07-22)
- ~~Bookmark/Board merge~~ → unified MyBoardScreen with bookmarkStore rewrite (2026-07-22)

## 16. Style naming reference

A vocabulary for invoking design patterns by name. See `SAFAR_STYLE_NAMES.md` (PDF also generated) for the full reference with specs, exemplar screens, and status (Stable/Provisional/Parked). Key names:

**Headers:** Pillar Hub Header (image-backed, ToolsScreen exemplar) · Sage Ornate Header (gold pattern, PrayerTimesScreen exemplar) · Sage Solid Header (CalendarScreen exception)

**Rows:** Tools Row (icon box + label + sub + caret, ToolsScreen exemplar) · Currency Row (Tools Row variant with amount/rate)

**Cards:** SafarAssist Card (shared component) · Pillar Card (HomeScreen 2×2) · Pillar Row (PillarList component) · Hero Card (HomeScreen top) · SAR Card (CurrencyScreen)

**Ornaments:** Ornament Divider (line · star · line) · Star Ornament (8-pointed gold star) · Header Pattern (gold SVG, HeaderPatternBg component)

**Shared components at project root:** HeaderPatternBg.jsx · SafarAssistCard.jsx · PillarList.jsx · KaabahIcon.jsx · flagAssets.js · bookmarkStore.js

## 17. Navigation fixes log (2026-07-18 through 2026-07-22)

- **Prepare-tab-showing-Bookmarks bug** — ToolsScreen Bookmarks row used cross-tab nav without `initial: false`, causing Prepare tab to show Bookmarks instead of its root screen. Fixed with `initial: false` on all cross-tab navigate calls.
- **Calendar-back-to-home bug** — CalendarScreen lacked `returnToTab` pattern. Added `route` to props, back button checks `route?.params?.returnToTab`.
- **My Journey card links (HomeScreen)** — Board and Contacts use cross-tab nav to Journey stack with `returnToTab: "Home"`. Checklist points to ChecklistsScreen (was incorrectly pointing to WhatToExpect). Arrow button goes to Journey tab.
- **MyContactsScreen back button** — added `returnToTab` pattern (was using plain `goBack()` which stayed in Journey stack after cross-tab nav from Home).
- **MyBoardScreen back button** — supports `returnToTab` from any calling tab (Home, Prepare, Tools, Plan hub, Connect hub).
- **ToolsScreen "Bookmarks" row** → renamed to "My Board" with PushPin icon, navigates to Journey stack MyBoard with `returnToTab: "Tools"`.
- **HubContainerScreen Plan hub "Bookmarks" row** → renamed to "My Board", navigates to Journey stack MyBoard.
