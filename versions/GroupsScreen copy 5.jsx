...
import {
  ...
  Alert, Animated, PanResponder, Image,
} from "react-native";
...
const EX_MILESTONES = {
  ex1: [
    { id:"m1", author:"Ahmed Al-Rashid", uid:"u2", text:"Completed Tawaf al-Qudum, alhamdulillah. 7 rounds done \uD83D\uDD4B", time:"2h ago", ameen:["u3","u4"], count:2, link:null, image:null },
    { id:"m2", author:"Fatima Hassan",   uid:"u1", text:"Making dua at Maqam Ibrahim right now. Please share your duas \uD83E\uDD32", time:"4h ago", ameen:["u2"], count:1, link:"https://sunnah.com/bukhari:1613", linkTitle:"Dua for Tawaf — Sunnah.com", image:null },
    { id:"m3", author:"Maryam Khan",     uid:"u3", text:"First time seeing the Kaaba — words cannot describe. Subhanallah.", time:"6h ago", ameen:["u1","u2","u4"], count:3, link:null, image:null },
  ],
  ex2: [
    { id:"m4", author:"Zainab Ali",   uid:"u5", text:"Just completed Sa\u02bfy \u2014 7 passes between Safa and Marwah. Alhamdulillah \uD83C\uDF3F", time:"1h ago", ameen:["u3","u4","u6"], count:3, link:null, image:null },
    { id:"m5", author:"Noor Ibrahim", uid:"u6", text:"Drank Zamzam water for the first time. Made dua for all of you \uD83D\uDCA7", time:"3h ago", ameen:["u5"], count:1, link:null, image:null },
  ],
  ex3: [
    { id:"m6", author:"Tariq Hassan", uid:"u7", text:"Arrived in Makkah safely after 14 hours of travel. Ya Allah \uD83D\uDD4B", time:"5h ago", ameen:["u2","u4","u8"], count:3, link:null, image:null },
    { id:"m7", author:"Omar Siddiq",  uid:"u8", text:"Entered Ihram at the Miqat. Labbayk All\u0101humma labbayk.", time:"8h ago", ameen:["u7"], count:1, link:null, image:null },
  ],
};
  ...
  return (
    <View style={ms.wrap}>
      ...
      <Animated.View style={[ms.card, { backgroundColor:tintBg, borderColor:userColor+"30", transform:[{translateX}] }]}
        ...
        {/* Image attachment */}
        {item.image && (
          <Image source={{ uri:item.image }} style={ms.image} resizeMode="cover" />
        )}

        {/* Link attachment */}
        {item.link && (
          <TouchableOpacity style={ms.linkChip}
            onPress={() => { const { Linking } = require("react-native"); Linking.openURL(item.link); }}
            activeOpacity={0.85}>
...
const ms = StyleSheet.create({
  ...
  meta:          { flex:1 },
  name:          { fontFamily:SERIF, fontSize:typography.small, fontWeight:"500", color:colors.text },
  time:          { fontSize:typography.tiny, color:colors.subtext },
  text:          { fontSize:typography.body, color:colors.text, lineHeight:22, marginBottom:spacing(1.25) },
  image:         { width:"100%", height:180, borderRadius:radius.sm, marginBottom:spacing(1.25) },
  ...
  linkChipText:  { flex:1, fontSize:typography.tiny, color:colors.primary, fontWeight:"400" },
  linkChipArrow: { fontSize:12, color:colors.primary },
  footer:        { flexDirection:"row", alignItems:"center", justifyContent:"space-between" },
  btn:           { flexDirection:"row", alignItems:"center", gap:spacing(0.75), paddingHorizontal:spacing(1.5), paddingVertical:spacing(0.75), borderRadius:radius.pill, borderWidth:1, borderColor:colors.border, backgroundColor:"rgba(255,255,255,0.5)" },
  ...
  swipeHint:     { fontSize:9, color:colors.border },
});
...
export default function GroupsScreen({ navigation }) {
  ...
  const handlePost = async () => {
    ...
    if (isExampleGroup) {
      const newM = {
        ...
        ameen:[], count:0, link, linkTitle:lTitle, image:postImage,
      };
  ...
  const pickImage = async () => {
    ...
    if (status !== "granted") { Alert.alert("Permission needed","Please allow photo access to attach images."); return; }
    ...
    if (!result.canceled && result.assets?.[0]) {
      ...
      if (asset.fileSize && asset.fileSize > MAX_IMAGE_MB * 1024 * 1024) {
        Alert.alert("Image too large", `Please choose an image under ${MAX_IMAGE_MB}MB.`);
        return;
      }
      ...
      <Modal visible={showPost} transparent animationType="slide">
        <KeyboardAvoidingView style={{ flex:1 }} behavior={Platform.OS==="ios"?"padding":"height"}>
          <TouchableOpacity style={mo.overlay} activeOpacity={1}
            onPress={() => { setShowPost(false); setPostText(""); setPostLink(""); setPostImage(null); setShowLinkField(false); }}>
            <View style={mo.sheet} onStartShouldSetResponder={() => true}>
              ...
              <View style={mo.attachRow}>
                <Text style={mo.attachLabel}>Add:</Text>
                ...
                <TouchableOpacity style={mo.attachBtn} onPress={pickImage} activeOpacity={0.8}>
                  <Text style={[mo.attachBtnText, postImage && mo.attachBtnActive]}>{"\uD83D\uDDBC\uFE0F"} Photo</Text>
                </TouchableOpacity>
                {postImage && (
                  <TouchableOpacity onPress={() => setPostImage(null)}>
                    <Text style={mo.removeAttach}>{"\u2715"} Remove photo</Text>
                  </TouchableOpacity>
                )}
              </View>
              ...
              {/* Image preview */}
              {postImage && (
                <Image source={{ uri:postImage }} style={mo.imagePreview} resizeMode="cover" />
              )}

              <Text style={mo.imageLimitNote}>Photos up to {MAX_IMAGE_MB}MB</Text>
...
const createStyles = (colors) => StyleSheet.create({
  ...
  newBtn:     { backgroundColor:colors.primary, borderRadius:radius.pill, paddingHorizontal:spacing(1.75), paddingVertical:spacing(0.875), ...shadows.button },
  ...
  memberName:       { fontSize:typography.tiny, color:colors.text, textAlign:"center", fontWeight:"400" },
  memberTapHint:    { fontSize:8, color:colors.border },
  ...
  hint:       { fontSize:typography.tiny, color:colors.subtext, marginBottom:spacing(1.5), lineHeight:17 },
...
const mo = StyleSheet.create({
  ...
  attachBtnActive:{ color:colors.primary, fontWeight:"600" },
  ...
  imagePreview:   { width:"100%", height:140, borderRadius:radius.sm, marginBottom:spacing(0.5) },
  imageLimitNote: { fontSize:9, color:colors.subtext, marginBottom:spacing(1.25), textAlign:"right" },
  ...
  cancel:         { flex:1, borderRadius:radius.md, borderWidth:1, borderColor:colors.border, paddingVertical:spacing(1.75), alignItems:"center", backgroundColor:colors.card },
  cancelText:     { fontSize:typography.body, color:colors.text },
  submit:         { flex:1, borderRadius:radius.md, backgroundColor:colors.primary, paddingVertical:spacing(1.75), alignItems:"center", ...shadows.button },
...
const ma = StyleSheet.create({
  ...
  memberSub:  { fontSize:typography.small, color:colors.subtext },
  divider:    { height:1, backgroundColor:colors.border, marginBottom:spacing(1.5) },
  sectionLabel:{ fontSize:10, fontWeight:"700", letterSpacing:1.5, color:colors.subtext, marginBottom:spacing(1) },
