                ...
                {cardType === "dua" && (
                  <>
                    ...
                    {filteredDuas.map(dua => (
                      <TouchableOpacity key={dua.id} style={[am.duaRow, selectedDua?.id===dua.id && am.duaRowSelected]}
                        onPress={() => setSelectedDua(dua)} activeOpacity={0.85}>
                        <View style={[am.duaCheck, selectedDua?.id===dua.id && am.duaCheckOn]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={cm.colorRow}>
                {CONTACT_COLORS.map((col, i) => (
                  <TouchableOpacity key={i} style={[cm.colorSwatch, { backgroundColor:col }, colorIndex===i && cm.colorSwatchActive]}
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={cm.roleRow}>
                {CONTACT_ROLES.map(r => (
                  <TouchableOpacity key={r} style={[cm.roleChip, role===r && cm.roleChipActive]}
                    onPress={() => setRole(r)} activeOpacity={0.8}>
                    <Text style={[cm.roleChipText, role===r && cm.roleChipTextActive]}>{r}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
...
function BoardCard({ card, onToggle, onDelete }) {
  ...
  if (card.type === "checklist") return (
    <TouchableOpacity style={[bc.card, bc.checklistCard]} onPress={() => onToggle(card.id)} onLongPress={handleLongPress} activeOpacity={0.85}>
      <View style={[bc.checkbox, card.done && bc.checkboxDone]}>
        {card.done && <Text style={bc.checkmark}>{"✓"}</Text>}
      </View>
      <Text style={[bc.checklistText, card.done && bc.checklistTextDone]}>{card.text}</Text>
    </TouchableOpacity>
  );
...
export default function JourneyScreen({ navigation }) {
  ...
  const saveBoard    = cards => { setBoardCards(cards); AsyncStorage.setItem(BOARD_KEY, JSON.stringify(cards)).catch(() => {}); };
  const saveContacts = list  => { setContacts(list);    AsyncStorage.setItem(CONTACTS_KEY, JSON.stringify(list)).catch(() => {}); };

  const addCard    = card    => { saveBoard([card, ...boardCards]); setShowAddCard(false); setActiveTab("board"); };
  const toggleCard = id      => saveBoard(boardCards.map(c => c.id===id ? {...c, done:!c.done} : c));
  const deleteCard = id      => saveBoard(boardCards.filter(c => c.id!==id));

  const addContact    = contact => saveContacts([...contacts, { ...contact, id:Date.now().toString() }]);
  const deleteContact = id      => saveContacts(contacts.filter(c => c.id!==id));
  const moveContact   = (index, dir) => {
    const next = [...contacts];
  ...
  const boardDone      = boardCards.filter(c => c.type==="checklist" && c.done).length;
    ...
    <SafeAreaView style={jn.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }}>
        ...
        <View style={jn.modeWrap}>
          <View style={jn.modeRow}>
            {[{key:"umrah",emoji:"\uD83D\uDD4B",name:"Umrah",sub:"Lesser pilgrimage"},{key:"hajj",emoji:"\u26F0\uFE0F",name:"Hajj",sub:"Annual pilgrimage"}].map(m => (
              <TouchableOpacity key={m.key} style={[jn.modeOpt, mode===m.key && jn.modeOptActive]}
                onPress={() => { setMode(m.key); setActiveTab("all"); }} activeOpacity={0.8}>
                <Text style={jn.modeEmoji}>{m.emoji}</Text>
                <Text style={[jn.modeName, mode===m.key && jn.modeNameActive]}>{m.name}</Text>
                <Text style={[jn.modeSub,  mode===m.key && jn.modeSubActive]}>{m.sub}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        {[
          ...
          { id:"board",    icon:"\uD83D\uDCCB", label:"My Journey Board",    sub: boardCards.length===0 ? "Notes, duas, checklists & links" : boardCards.length + " card" + (boardCards.length===1?"":"s") + (boardChecklist>0?" \u00b7 "+boardDone+"/"+boardChecklist+" tasks done":""), onPress:() => setActiveTab("board") },
          { id:"contacts", icon:"\uD83D\uDC64", label:"My Contacts",         sub: contacts.length===0 ? "Hotel, guide, driver & group members" : contacts.length + " contact" + (contacts.length===1?"":"s") + " saved", onPress:() => setActiveTab("contacts") },
        ].map(item => (
        ...
        {/* My Board / Contacts tab bar */}
        <View style={jn.tabs}>
          {[["board","My Board"],["contacts","Contacts"]].map(([key,label]) => (
            <TouchableOpacity key={key} style={[jn.tab, activeTab===key && jn.tabActive]}
              ...
              {key==="board" && boardCards.length>0 && <View style={jn.tabBadge}><Text style={jn.tabBadgeText}>{boardCards.length}</Text></View>}
              {key==="contacts" && contacts.length>0 && <View style={jn.tabBadge}><Text style={jn.tabBadgeText}>{contacts.length}</Text></View>}
            </TouchableOpacity>
          ))}
        </View>
        ...
        {/* My Board */}
        {activeTab === "board" && (
          <View style={jn.boardWrap}>
            <View style={jn.boardHeader}>
              <View>
                <Text style={jn.boardTitle}>My Journey Board</Text>
                {boardChecklist > 0 && <Text style={jn.boardProgress}>{boardDone} of {boardChecklist} tasks done</Text>}
              </View>
              <TouchableOpacity style={jn.addCardBtn} onPress={() => setShowAddCard(true)} activeOpacity={0.85}>
                <Text style={jn.addCardBtnText}>{"+ Add"}</Text>
              </TouchableOpacity>
            </View>
            {boardChecklist > 0 && (
              <View style={jn.boardProgTrack}>
                <View style={[jn.boardProgFill, { width:(boardDone/boardChecklist*100)+"%" }]} />
              </View>
            )}
            {boardCards.length === 0 ? (
              <View style={jn.boardEmpty}>
                <Text style={jn.boardEmptyEmoji}>{"\uD83D\uDCCB"}</Text>
                <Text style={jn.boardEmptyTitle}>Your board is empty</Text>
                <Text style={jn.boardEmptyBody}>Add notes, checklist items, pinned du\u02bf\u0101\u02beS and links to personalise your journey.</Text>
                <TouchableOpacity style={jn.boardEmptyBtn} onPress={() => setShowAddCard(true)} activeOpacity={0.85}>
                  <Text style={jn.boardEmptyBtnText}>{"+ Add your first card"}</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <>
                {boardCards.map(card => <BoardCard key={card.id} card={card} onToggle={toggleCard} onDelete={deleteCard} />)}
                <Text style={jn.boardHint}>Long press any card to remove it</Text>
              </>
            )}
          </View>
        )}
        ...
        {activeTab === "contacts" && (
          <View style={jn.boardWrap}>
            <View style={jn.boardHeader}>
              <View>
                <Text style={jn.boardTitle}>My Contacts</Text>
                <Text style={jn.boardProgress}>
                  {contacts.length === 0 ? "Hotel, guide, driver, group members" : contacts.length + " contact" + (contacts.length===1?"":"s")}
                </Text>
              </View>
              <TouchableOpacity style={jn.addCardBtn} onPress={() => setShowAddContact(true)} activeOpacity={0.85}>
                <Text style={jn.addCardBtnText}>{"+ Add"}</Text>
              </TouchableOpacity>
            </View>
            ...
            {contacts.length === 0 ? (
              <View style={jn.boardEmpty}>
                <Text style={jn.boardEmptyEmoji}>{"\uD83D\uDCDE"}</Text>
                <Text style={jn.boardEmptyTitle}>No contacts yet</Text>
                <Text style={jn.boardEmptyBody}>Pin your hotel, travel agent, local guide, driver, or group members so everything is in one place when you need it.</Text>
                <TouchableOpacity style={jn.boardEmptyBtn} onPress={() => setShowAddContact(true)} activeOpacity={0.85}>
                  <Text style={jn.boardEmptyBtnText}>{"+ Add first contact"}</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <>
                <Text style={jn.contactsHint}>Tap a phone number to call. Use arrows to reorder by priority.</Text>
                {contacts.map((contact, index) => (
                  <View key={contact.id} style={jn.contactCard}>
                    <View style={[jn.contactAvatar, { backgroundColor:CONTACT_COLORS[contact.colorIndex ?? 0] }]}>
                      <Text style={jn.contactAvatarText}>
                        {contact.name?.split(" ").map(w=>w[0]).slice(0,2).join("").toUpperCase() ?? "?"}
                      </Text>
                    </View>
                    <View style={jn.contactInfo}>
                      <View style={jn.contactTopRow}>
                        <Text style={jn.contactName}>{contact.name}</Text>
                        <View style={[jn.contactRolePill, { backgroundColor:ROLE_COLORS[contact.role] ?? "rgba(47,93,80,0.10)" }]}>
                          <Text style={jn.contactRoleText}>{contact.role}</Text>
                        </View>
                      </View>
                      {contact.phone ? (
                        <TouchableOpacity onPress={() => Linking.openURL("tel:"+contact.phone)} activeOpacity={0.7}>
                          <Text style={jn.contactPhone}>{"\uD83D\uDCDE"} {contact.phone}</Text>
                        </TouchableOpacity>
                      ) : null}
                      {contact.note ? <Text style={jn.contactNote} numberOfLines={2}>{contact.note}</Text> : null}
                    </View>
                    <View style={jn.contactControls}>
                      <TouchableOpacity style={jn.contactArrowBtn} onPress={() => moveContact(index, -1)} disabled={index===0} activeOpacity={0.7}>
                        <Text style={[jn.contactArrowText, index===0 && jn.contactArrowDisabled]}>{"\u2191"}</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={jn.contactArrowBtn} onPress={() => moveContact(index, 1)} disabled={index===contacts.length-1} activeOpacity={0.7}>
                        <Text style={[jn.contactArrowText, index===contacts.length-1 && jn.contactArrowDisabled]}>{"\u2193"}</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={jn.contactArrowBtn} onPress={() => {
                        Alert.alert("Remove contact","Remove "+contact.name+"?",[
                          {text:"Cancel",style:"cancel"},
                          {text:"Remove",style:"destructive",onPress:()=>deleteContact(contact.id)},
                        ]);
                      }} activeOpacity={0.7}>
                        <Text style={[jn.contactArrowText, { color:colors.border }]}>{"\u2715"}</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                ))}
                <Text style={jn.boardHint}>Long press or tap {"\u2715"} to remove</Text>
              </>
            )}
          </View>
        )}
      ...
      <AddCardModal    visible={showAddCard}    onAdd={addCard}    onClose={() => setShowAddCard(false)}    />
      <AddContactModal visible={showAddContact} onAdd={addContact} onClose={() => setShowAddContact(false)} />

      {/* FAB */}
      {(activeTab==="board" && boardCards.length>0) || (activeTab==="contacts" && contacts.length>0) ? (
        <TouchableOpacity style={jn.fab}
          onPress={() => activeTab==="contacts" ? setShowAddContact(true) : setShowAddCard(true)}
          activeOpacity={0.88}>
          <Text style={jn.fabText}>{"+"}</Text>
        </TouchableOpacity>
      ) : null}
...
const am = StyleSheet.create({
  ...
  suggestTitle:{ fontSize:10, fontWeight:"700", letterSpacing:1.5, color:colors.subtext, marginBottom:spacing(0.75) },
  ...
  suggestionText:{ fontSize:typography.small, color:colors.primary, fontWeight:"400" },
  ...
  duaRowSelected:{ backgroundColor:"rgba(47,93,80,0.06)", borderRadius:radius.sm },
...
const jn = StyleSheet.create({
  ...
  tab:            { flex:1, paddingVertical:spacing(1), borderRadius:radius.sm, alignItems:"center", flexDirection:"row", justifyContent:"center", gap:4 },
  tabActive:      { backgroundColor:colors.card, ...shadows.card },
  tabLabel:       { fontSize:9, color:colors.subtext, fontWeight:"300" },
  ...
  // Board + Contacts shared layout
  boardWrap:         { marginHorizontal:spacing(2.5) },
  boardHeader:       { flexDirection:"row", alignItems:"center", justifyContent:"space-between", marginBottom:spacing(1) },
  boardTitle:        { fontFamily:SERIF, fontSize:typography.heading, color:colors.text },
  boardProgress:     { fontSize:typography.tiny, color:colors.primary, fontWeight:"500", marginTop:2 },
  addCardBtn:        { backgroundColor:colors.primary, borderRadius:radius.pill, paddingHorizontal:spacing(1.75), paddingVertical:spacing(0.875), ...shadows.button },
  addCardBtnText:    { fontSize:typography.small, color:"#fff", fontWeight:"500" },
  boardProgTrack:    { height:4, backgroundColor:colors.border, borderRadius:2, marginBottom:spacing(1.75), overflow:"hidden" },
  boardProgFill:     { height:"100%", backgroundColor:colors.primary, borderRadius:2 },
  boardEmpty:        { alignItems:"center", paddingVertical:spacing(4), paddingHorizontal:spacing(2) },
  boardEmptyEmoji:   { fontSize:44, marginBottom:spacing(1.5) },
  boardEmptyTitle:   { fontFamily:SERIF, fontSize:20, color:colors.text, marginBottom:spacing(1) },
  boardEmptyBody:    { fontSize:typography.body, color:colors.subtext, textAlign:"center", lineHeight:22, marginBottom:spacing(2.5) },
  boardEmptyBtn:     { backgroundColor:colors.primary, borderRadius:radius.md, paddingVertical:spacing(1.5), paddingHorizontal:spacing(3), ...shadows.button },
  boardEmptyBtnText: { color:"#fff", fontWeight:"500" },
  boardHint:         { fontSize:typography.tiny, color:colors.border, textAlign:"center", marginTop:spacing(1.5), marginBottom:spacing(1) },

  // Contacts
  contactsHint:     { fontSize:typography.tiny, color:colors.subtext, fontWeight:"300", marginBottom:spacing(1.5), lineHeight:17 },
  contactCard:      { flexDirection:"row", alignItems:"center", gap:spacing(1.25), backgroundColor:colors.card, borderRadius:radius.md, borderWidth:1, borderColor:colors.border, padding:spacing(1.75), marginBottom:spacing(1.25), ...shadows.card },
  contactAvatar:    { width:44, height:44, borderRadius:22, alignItems:"center", justifyContent:"center", flexShrink:0 },
  contactAvatarText:{ fontSize:16, color:"#fff", fontWeight:"700" },
  contactInfo:      { flex:1 },
  contactTopRow:    { flexDirection:"row", alignItems:"center", gap:spacing(0.75), marginBottom:3 },
  contactName:      { fontFamily:SERIF, fontSize:typography.body, color:colors.text, flex:1 },
  contactRolePill:  { borderRadius:radius.pill, paddingHorizontal:spacing(0.875), paddingVertical:2 },
  contactRoleText:  { fontSize:9, color:colors.primary, fontWeight:"600" },
  contactPhone:     { fontSize:typography.small, color:colors.primary, fontWeight:"400", marginBottom:2 },
  contactNote:      { fontSize:typography.tiny, color:colors.subtext, fontWeight:"300", lineHeight:16 },
  contactControls:  { flexDirection:"column", gap:4, alignItems:"center" },
  contactArrowBtn:  { width:28, height:28, borderRadius:14, backgroundColor:colors.background, borderWidth:1, borderColor:colors.border, alignItems:"center", justifyContent:"center" },
  contactArrowText: { fontSize:14, color:colors.primary, fontWeight:"600" },
  contactArrowDisabled:{ color:colors.border },
