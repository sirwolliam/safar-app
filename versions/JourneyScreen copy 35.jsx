...
export default function JourneyScreen({ navigation }) {
  ...
  const markedDates = {
    ...
    [departureDateStr]: {
      ...
      marked: HAJJ_KEY_DATES[departureDateStr] !== undefined,
      dotColor: "#C8A96A",
    },
  };
  ...
  const handleDayPress = (day) => {
    ...
    // Show label if it's a Hajj key date
  ...
  const departureDateFormatted = new Date(departureDateStr).toLocaleDateString("en-GB", {
    ...
    <SafeAreaView style={jn.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }} contentContainerStyle={jn.scroll}>
        ...
        <View style={jn.pageHeader}>
          ...
          <Text style={jn.pageSub}>Your hub to plan and prepare every step with confidence.</Text>
        </View>
        ...
        <TouchableOpacity style={jn.datesCard} onPress={() => setShowCalendar(true)} activeOpacity={0.88}>
          <View style={jn.datesCardLeft}>
            <Text style={jn.datesCardEyebrow}>YOUR JOURNEY</Text>
            <Text style={jn.datesCardTitle}>Mark Your Dates</Text>
            <Text style={jn.datesCardSub}>Set your departure and follow key Hajj dates on the calendar</Text>
          </View>
          <View style={jn.datesCardRight}>
            <View style={jn.datesCardIcon}>
              <Text style={jn.datesCardIconTxt}>{"📅"}</Text>
            </View>
            <Text style={jn.datesCardDate}>{departureDateFormatted}</Text>
            ...
            <View style={jn.calSheet}>
              ...
              <View style={jn.calHeader}>
                <View>
                  ...
                  <Text style={jn.calSub}>Tap a date to set your departure</Text>
                </View>
                <TouchableOpacity style={jn.calCloseBtn} onPress={() => setShowCalendar(false)}>
              ...
              {/* Selected day label — shows for Hajj key dates */}
              {selectedDayLabel && (
        ...
        <TouchableOpacity style={jn.checklistHeader} onPress={() => setChecklistOpen(v => !v)} activeOpacity={0.85}>
          <ImageBackground
            source={mode === "hajj" ? require("../assets/07_arafah_gradient.jpg") : require("../assets/Umrah_02_arrival_gradient.jpg")}
...
const jn = StyleSheet.create({
  ...
  // ── Page header ─────────────────────────────────────────────────────────────
  pageHeader:    { paddingTop:20, paddingBottom:16, paddingHorizontal:2 },
  pageTitleRow:  { flexDirection:"row", alignItems:"flex-start", justifyContent:"space-between", marginBottom:6 },
  pageTitle:     { fontFamily:SERIF, fontSize:32, fontWeight:"400", color:"#1A1712", lineHeight:38, flex:1 },
  pageSub:       { fontSize:14, color:"#5A5650", lineHeight:22, fontWeight:"400" },
  departureBadge:{ alignItems:"center", backgroundColor:"#FDFAF4", borderRadius:10, borderWidth:1, borderColor:"#C4A882", paddingHorizontal:12, paddingVertical:7, marginLeft:12, shadowColor:"#6A4A28", shadowOffset:{width:0,height:2}, shadowOpacity:0.10, shadowRadius:4, elevation:2 },
  departureDays: { fontFamily:SERIF, fontSize:20, color:"#1A1712", fontWeight:"400", lineHeight:24 },
  ...
  // ── Dates card ──────────────────────────────────────────────────────────────
  datesCard:        { flexDirection:"row", alignItems:"center", backgroundColor:"#FDFAF4", borderRadius:16, borderWidth:1, borderColor:"#D4D0CA", marginBottom:12, padding:18, shadowColor:"#6A4A28", shadowOffset:{width:0,height:3}, shadowOpacity:0.12, shadowRadius:8, elevation:4, gap:16 },
  ...
  datesCardRight:   { alignItems:"center", gap:4 },
  datesCardIcon:    { width:44, height:44, borderRadius:22, backgroundColor:"#EBF2EE", alignItems:"center", justifyContent:"center" },
  datesCardIconTxt: { fontSize:22 },
  datesCardDate:    { fontFamily:SERIF, fontSize:13, color:"#2F5D50", textAlign:"center" },
  datesCardDays:    { fontSize:11, color:"#9A8E7A", textAlign:"center" },

  // ── Calendar sheet ───────────────────────────────────────────────────────────
  calOverlay:      { flex:1, backgroundColor:"rgba(0,0,0,0.45)", justifyContent:"flex-end" },
  calSheet:        { backgroundColor:"#FDFAF4", borderTopLeftRadius:24, borderTopRightRadius:24, paddingHorizontal:20, maxHeight:"85%" },
  calHandle:       { width:36, height:4, borderRadius:2, backgroundColor:"#D4D0CA", alignSelf:"center", marginTop:12, marginBottom:4 },
  calHeader:       { flexDirection:"row", alignItems:"flex-start", justifyContent:"space-between", paddingVertical:14 },
