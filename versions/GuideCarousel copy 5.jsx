...
import {
  ...
  ImageBackground, Dimensions, FlatList, StatusBar,
} from "react-native";
...
function Slide({ step, total, guideTitle }) {
  return (
    <View style={sl.slide}>
      {/* Image — 100% fill, no colour overlay */}
      <ImageBackground
        source={step.image}
      ...
      {/* Built-in gradient — top dark band only, no bottom overlay */}
      <View style={sl.gradTop} />

      {/* Guide title — gold, centred, top */}
      <View style={sl.topContent}>
        <Text style={sl.guideTitle}>{guideTitle.toUpperCase()}</Text>

        {/* Step number — large, centred */}
        <Text style={sl.stepNum}>{String(step.num).padStart(2, "0")}</Text>
        <Text style={sl.stepOf}>{"of " + total}</Text>
      </View>

      {/* Step title block — centred vertically */}
      <View style={sl.midContent}>
      ...
      {/* Source note — bottom, small, translucent pill */}
...
const sl = StyleSheet.create({
  ...
  gradTop:    {
    position:"absolute", top:0, left:0, right:0, height:SH * 0.42,
    backgroundColor:"rgba(8,16,10,0.62)",
  },
  topContent: { position:"absolute", top:60, left:0, right:0, alignItems:"center", paddingHorizontal:32 },
  guideTitle: { fontSize:11, fontWeight:"800", letterSpacing:3, color:"#C8A96A", marginBottom:16 },
  stepNum:    { fontFamily:SERIF, fontSize:72, color:"#fff", lineHeight:76, fontWeight:"300" },
  stepOf:     { fontSize:13, color:"rgba(255,255,255,0.50)", fontWeight:"500", marginTop:4, letterSpacing:0.5 },
  midContent: { position:"absolute", top:SH * 0.42, left:0, right:0, paddingHorizontal:32, paddingTop:24, alignItems:"center" },
  subtitle:   { fontSize:12, fontWeight:"700", letterSpacing:2.5, color:"#C8A96A", textAlign:"center", marginBottom:10, textTransform:"uppercase" },
  divider:    { width:32, height:1.5, backgroundColor:"rgba(255,255,255,0.35)", marginBottom:12 },
  title:      { fontFamily:SERIF, fontSize:28, color:"#fff", textAlign:"center", marginBottom:14, lineHeight:34 },
  body:       { fontSize:15, color:"rgba(255,255,255,0.85)", lineHeight:24, textAlign:"center", fontWeight:"400" },
  sourceWrap: { position:"absolute", bottom:40, left:32, right:32, alignItems:"center" },
  sourcePill: { backgroundColor:"rgba(0,0,0,0.30)", borderRadius:12, borderWidth:1, borderColor:"rgba(255,255,255,0.12)", paddingHorizontal:14, paddingVertical:6 },
  sourceTxt:  { fontSize:9, color:"rgba(255,255,255,0.45)", textAlign:"center", lineHeight:14, fontWeight:"400", letterSpacing:0.3 },
});
