...
const LIBRARY_CATEGORIES = [
  ...
  { id:"protect",   name:"Protection",           count:20, emoji:"\uD83D\uDEE1\uFE0F", shade:"#A8C8B8" },
...
// ── Shared tab bar used across all 3 views ────────────────────────────────────
function TabBar({ active, onSelect }) {
  const tabs = [
    { key:"myLists", label:"My Lists",    sub:"Lists you\u2019ve saved" },
    { key:"library", label:"Dua Library", sub:"Browse all duas"       },
    { key:"shared",  label:"Shared Duas", sub:"Shared with me"        },
  ];
  return (
    <View style={tb.row}>
      {tabs.map(t => (
...
const tb = StyleSheet.create({
  ...
  btn:    { flex:1, backgroundColor:colors.card, borderRadius:radius.md, borderWidth:1, borderColor:colors.border, padding:spacing(1.25), alignItems:"center", gap:2 },
  btnOn:  { backgroundColor:colors.primary, borderColor:colors.primary },
  label:  { fontFamily:SERIF, fontSize:10, color:colors.text, textAlign:"center" },
  labelOn:{ color:"#fff", fontWeight:"600" },
  sub:    { fontSize:8, color:colors.subtext, textAlign:"center", lineHeight:12 },
  subOn:  { color:"rgba(255,255,255,0.75)" },
});
        ...
        <View style={nl.overlay}>
          <View style={nl.sheet}>
            ...
            <View style={nl.btnRow}>
              ...
              <TouchableOpacity
                style={!name.trim() ? [nl.createBtn, nl.createBtnDim] : nl.createBtn}
...
export default function MyDuasScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {/* Sticky header */}
      <View style={s.header}>
        <Text style={s.headerTitle}>My Du\u02bfA\u02beS</Text>
      ...
      {activeTab === "library" && (
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={s.searchBar}>
            ...
            <TextInput style={s.searchInput} placeholder="Search duas\u2026"
      ...
      {activeTab === "shared" && (
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          <View style={s.sharedBanner}>
            <Text style={s.sharedBannerTitle}>Shared Du\u02bf\u0101\u02beS</Text>
            <Text style={s.sharedBannerSub}>Du\u02bf\u0101\u02beS shared with you from groups and contacts</Text>
          </View>
          ...
          {SHARED_DUAS.length === 0 ? (
            <View style={s.empty}>
              ...
              <Text style={s.emptyBody}>When group members share dua lists with you, they\u2019ll appear here.</Text>
            </View>
          ) : (
            <View style={s.listContainer}>
              {SHARED_DUAS.map((item, idx) => (
                <TouchableOpacity key={item.id}
                  ...
                  onPress={() => navigation.navigate("DuaList", { list:{ id:item.id, name:item.name, count:item.count, emoji:item.emoji, gradientBot:"#2A3C30" } })}
                  activeOpacity={0.8}>
                  <View style={[s.listThumb, { backgroundColor:"#2A3C30" }]}>
                    <Text style={s.listThumbEmoji}>{item.emoji}</Text>
                  </View>
                  <View style={s.listInfo}>
                    <Text style={s.listName}>{item.name}</Text>
                    <Text style={s.listMeta}>From {item.sharedBy} \u00b7 {item.count} duas</Text>
                    <Text style={s.listMetaSub}>Updated {item.updated}</Text>
                  </View>
                  <Text style={s.listArrow}>{"\u203a"}</Text>
                </TouchableOpacity>
              ))}
...
const s = StyleSheet.create({
  ...
  scroll: { paddingHorizontal:spacing(2.5), paddingTop:spacing(1) },

  // List rows
  listContainer: { backgroundColor:colors.card, borderRadius:radius.lg, borderWidth:1, borderColor:colors.border, overflow:"hidden", marginBottom:spacing(2) },
  ...
  listThumb:     { width:50, height:50, borderRadius:radius.md, alignItems:"center", justifyContent:"center", flexShrink:0 },
  listThumbEmoji:{ fontSize:22 },
  listInfo:      { flex:1 },
  listName:      { fontFamily:SERIF, fontSize:typography.body, fontWeight:"500", color:colors.text, marginBottom:2 },
  listMeta:      { fontSize:typography.small, color:colors.subtext, fontWeight:"300" },
  listMetaSub:   { fontSize:typography.tiny, color:colors.border },
  ...
  // Library
  searchBar:       { flexDirection:"row", alignItems:"center", backgroundColor:colors.card, borderRadius:radius.md, paddingHorizontal:spacing(2), paddingVertical:spacing(1.25), gap:spacing(1), borderWidth:1, borderColor:colors.border, marginBottom:spacing(1.5) },
  ...
  filterChip:      { paddingHorizontal:spacing(1.75), paddingVertical:spacing(0.75), borderRadius:radius.pill, borderWidth:1, borderColor:colors.border },
  filterChipOn:    { backgroundColor:colors.primary, borderColor:colors.primary },
  filterChipTxt:   { fontSize:typography.small, color:colors.subtext, fontWeight:"300" },
  filterChipTxtOn: { color:"#fff", fontWeight:"500" },
  categoryGrid:    { flexDirection:"row", flexWrap:"wrap", gap:spacing(1.25) },
  categoryCard:    { width:"47.5%", borderRadius:radius.md, overflow:"hidden", borderWidth:1, borderColor:colors.border },
  ...
  // FAB
  fab:    { position:"absolute", bottom:spacing(3), right:spacing(2.5), width:50, height:50, borderRadius:25, backgroundColor:colors.primary, alignItems:"center", justifyContent:"center" },
