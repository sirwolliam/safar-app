...
function ContactCard({ contact, index, onDelete, onEdit }) {
        ...
        >
          ...
          {expanded && (
            <View style={[cc.expandedSection, { borderTopColor: accentColor + "22" }]}>
              ...
              {/* WhatsApp */}
              {contact.phone ? (
                <TouchableOpacity style={cc.expandRow} onPress={() => Linking.openURL("https://wa.me/" + contact.phone.replace(/[^0-9]/g, ""))} activeOpacity={0.8}>
                  <View style={[cc.expandIcon, { backgroundColor: "#25D36618" }]}>
                    <Text style={cc.expandIconTxt}>{"💬"}</Text>
                  </View>
                  <View style={cc.expandInfo}>
                    <Text style={cc.expandLabel}>WhatsApp</Text>
                    <Text style={[cc.expandValue, { color:"#25D366" }]}>{contact.phone}</Text>
                  </View>
                  <Text style={cc.expandArrow}>{"›"}</Text>
                </TouchableOpacity>
              ) : null}
...
function ContactModal({ visible, editContact, onSave, onClose }) {
  ...
  useEffect(() => {
    if (editContact) {
      ...
      setName(""); setRole("Hotel"); setPhone(""); setEmail(""); setNote(""); setColorIndex(0);
    }
  }, [editContact, visible]);
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      email: email.trim(), note: note.trim(), colorIndex,
    });
  };
